...
function setupSpendDial(D){
  ...
  p.innerHTML =
    ...
    + '<table class="sd-table" id="sdTable"><thead><tr><th>rank</th><th>ring ID</th><th>avg val</th><th>cov</th><th>cost-to-win</th><th>IS</th></tr></thead><tbody></tbody></table>';
  host.appendChild(p);
...
function sdRedrawMap(sol){           // v1 SNAP on Solve: highlight funded rings, grey deselected
  ...
  (sol.funded_circle_ids || []).forEach(cid => { const c = SD_CIRCLES[cid]; if(!c) return;
    layers.push(L.circle([c.lat, c.lng], { pane:"sdSnap", radius: c.r*1609.344, color:"#16a34a",
      weight:3, opacity:.95, fill:false })); });
...
async function boot(){
  ...
  if(TMAP){
    ...
    if(D.swath_mode==="per_cell" && D.swath_cells && D.swath_cells.length){
      const h=buildHeatmap(D.swath_cells,HAIL_FILL[curHailMode()].fn,HAIL_RAMP,"hHeatPane",351);
    ...
    if(HL.length){ let mode="smooth";
      ...
      const rebuildHail=()=>{ const o=HL.find(x=>x.peril==="hail"); if(!o)return;
        ...
        const g=buildHeatmap(D.swath_cells,HAIL_FILL[curHailMode()].fn,HAIL_RAMP,"hHeatPane",351);
