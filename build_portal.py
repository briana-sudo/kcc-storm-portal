...
function sgDrawReshape(moves){
  ...
  try{
    ...
    moves.forEach(function(m){
      ...
      if(bc) L.circle([bc[0], bc[1]], {radius: br*1609.34, color:"#94a3b8", weight:1, fill:false}).addTo(SG_RESHAPE_LAYER);
      if(ac && (m.new_center || m.new_radius_mi)) L.circle([ac[0], ac[1]], {radius: ar*1609.34, color:"#4ade80", weight:2, dashArray:"5,4", fill:false}).addTo(SG_RESHAPE_LAYER);
    });
    SG_RESHAPE_LAYER.addTo(TMAP);
  }catch(e){ console.error("[reshape] draw failed", e); }
}
...
async function sgReshapeWinCheap(){
  ...
  catch(e){ if(v){ v.textContent="Reshape preview failed \\u2014 "+e.message; v.className="sd-verdict warn"; } return; }
