...
/* expandable map (desktop): hide the side panel so the map fills the area */
...
SHELL_HEAD = """  <div id="tbar">
    ...
    <button id="expandBtn" title="Expand the map">&#9974; Expand</button>
...
async function boot(){
  ...
  if(eb) eb.onclick=()=>{ const f=document.body.classList.toggle("fullmap");
    eb.innerHTML=f?"\\u26f6 Collapse":"\\u26f6 Expand"; window.dispatchEvent(new Event("resize")); };
