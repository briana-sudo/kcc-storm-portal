...
function buildHeatmap(cells, valFn, ramp, vmax, paneName, paneZ){
...
async function boot(){
  ...
  if(TMAP){
    ...
    if(D.swath_mode==="per_cell" && D.swath_cells && D.swath_cells.length){
      const m=HAIL_FILL[curHailMode()]; const h=buildHeatmap(D.swath_cells,m.fn,HAIL_RAMP,m.vmax,"hHeatPane",351);
      if(h) HL.push({pane:"swathPane",group:h,peril:"hail",op:0.6}); }
    if(D.wind && D.wind.swath_cells && D.wind.swath_cells.length){
      const w=buildHeatmap(D.wind.swath_cells,c=>+c[2]||0,WIND_RAMP,10,"wHeatPane",354); if(w) HL.push({pane:"windPane",group:w,peril:"wind",op:0.55}); }
    if(D.tornado && D.tornado.swath_cells && D.tornado.swath_cells.length){
      const t=buildHeatmap(D.tornado.swath_cells,c=>+c[2]||0,TORN_RAMP,10,"tHeatPane",355); if(t) HL.push({pane:"tornadoPane",group:t,peril:"tornado",op:0.55}); }
    if(HL.length){ let mode="smooth";
      ...
      // Cells | Smooth — the ONLY portal-added control. Everything else (per-peril opacity,
      // fill mode, swath on/off) flows through the existing §3 Layers panel below.
      ...
      const rebuildHail=()=>{ const o=HL.find(x=>x.peril==="hail"); if(!o)return;
        const m=HAIL_FILL[curHailMode()]; if(TMAP.hasLayer(o.group)) TMAP.removeLayer(o.group);
        const g=buildHeatmap(D.swath_cells,m.fn,HAIL_RAMP,m.vmax,"hHeatPane",351);
