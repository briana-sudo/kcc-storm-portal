...
SHELL_HEAD = """  <div id="tbar">
    ...
    <div class="hdr-search">
      ...
      <button id="msCardBtn" class="ms-act" type="button" title="Re-open the address card for the resolved address" style="display:none">&#128196; Card</button>
...
function showTempCard(addr, lat, lng, opts){
  ...
  TC_LAST={addr:addr, lat:lat, lng:lng, opts:opts};   // retained for the Card relaunch button (no re-resolve)
  const _cb=document.getElementById("msCardBtn"); if(_cb) _cb.style.display="";
...
function addMapSearch(){
  ...
  // CARD RELAUNCH (2026-08-01): re-open the card from the RETAINED resolve state — no re-search, no re-resolve
  // unless the field text no longer matches the resolved address (stale).
  const cardBtn=document.getElementById("msCardBtn");
  if(cardBtn) cardBtn.addEventListener("click", ()=>{
    if(TC_LAST && addrEl.value.trim()===String(TC_LAST.addr).trim())
      showTempCard(TC_LAST.addr, TC_LAST.lat, TC_LAST.lng, TC_LAST.opts);   // retained state
    else if(addrEl.value.trim()) addrGo();                                  // stale -> re-resolve
  });
  ...
  document.querySelectorAll("#tbar .hdr-search .ms-clear").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    if(btn.dataset.clear==="msAddr"){ suggEl.innerHTML="";
      TC_LAST=null; const cb=document.getElementById("msCardBtn"); if(cb) cb.style.display="none";
