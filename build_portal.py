...
/* ── ON-MAP TIER LEGEND (2026-07-20) — collapsible, same vocabulary as the calendar popup so one
   legend teaches both surfaces. Sits top-left under the zoom control. ── */
#tierLegend{position:absolute;left:10px;top:78px;z-index:642;font:600 10.5px/1.35 system-ui,-apple-system,sans-serif;
...
function addChaseSwathLayer(D){
  ...
  if(hull.length>=3){
    ...
    // Anchor the label at the hull's NORTHERNMOST vertex so it sits clear of the swath body
    // and lands somewhere stable across dates (a centroid label would sit ON the cells).
    let top=hull[0]; for(const p of hull){ if(p[0]>top[0]) top=p; }
    layers.push(L.marker(top,{pane:"chaseSwathPane",interactive:false,
