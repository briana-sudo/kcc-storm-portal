...
let _HOVER = {idx:null, step:0.01, tip:null, wired:false};
...
function initSwathHover(cells){
  const live = (cells && cells.length && cells[0].length>=7) ? cells : null;  // 7-wide == live provisional
  _HOVER.idx = null;
  if(live){
    const lats=[...new Set(live.map(c=>+c[0]))].sort((a,b)=>a-b);
    let st=0.01; for(let i=1;i<lats.length;i++){ const d=lats[i]-lats[i-1]; if(d>1e-6 && d<st) st=d; }
    _HOVER.step = st||0.01;
    const m=new Map(); for(const c of live) m.set(_hoverKey(+c[0],+c[1],_HOVER.step), c); _HOVER.idx=m;
  }
  if(!TMAP || _HOVER.wired) return;
  _HOVER.wired=true;
  _HOVER.tip=L.tooltip({sticky:true, direction:"top", opacity:0.95, className:"swath-hover-tip"});
  TMAP.on("mousemove", e=>{
    const c=_HOVER.idx && _HOVER.idx.get(_hoverKey(e.latlng.lat, e.latlng.lng, _HOVER.step));
    if(!c){ TMAP.closeTooltip(_HOVER.tip); return; }
    const size=(+c[2]||0).toFixed(2), pers=(+c[4]||0);
    _HOVER.tip.setLatLng(e.latlng).setContent(
      '<b>'+size+'\\u2033 hail</b><br>max @ '+_fmtScanTs(c[6])+'<br>'+pers+' scan'+(pers===1?'':'s')+' with hail');
    TMAP.openTooltip(_HOVER.tip);
  });
...
async function boot(){
  ...
  if(TMAP){
    ...
    try{ initSwathHover(D.swath_cells); }catch(e){}   // Part C: live-swath hover (size/scan-time/persistence)
    try{ updateSwathBadge(D.swath_cells); }catch(e){} // render-truth: sub-gate/gated glance badge
