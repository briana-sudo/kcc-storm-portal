...
function initCampaigns(){
  ...
  function rowHtml(r,clickable){
    ...
    h+='<div class="cmp-mets">'+
       ...
       '<span>IS <b>'+pct(r.impression_share)+'</b></span>'+
       '<span>Spend(win) <b>'+money(r.spend_window)+'</b></span>'+
       '</div>';
    const p=r.pacing;
    if(p){ h+='<div class="cmp-pace">Today <b>'+money(p.spend_today)+'</b> / cap '+money(p.daily_budget)+
       (p.label? ' <span class="cmp-pill '+p.label+'">'+p.label+(p.ratio!=null?" "+Math.round(p.ratio*100)+"%":"")+'</span>':'')+'</div>'; }
  ...
  function renderDetail(rep){
    ...
    if(!rings.length){ h+='<div class="cmp-empty">No per-ring metrics in the window.</div>'; }
      ...
      h+='<div class="cmp-ring"><span class="r-idx">#'+r.idx+'</span><span class="r-mets">Impr <b>'+num(r.impressions)+'</b> \\u00b7 Clicks <b>'+num(r.clicks)+'</b> \\u00b7 IS <b>'+pct(r.impression_share)+'</b></span><span>'+money(r.spend)+'</span></div>'; });
      h+='</div>'; }
    body.innerHTML=h;
