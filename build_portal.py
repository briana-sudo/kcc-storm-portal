...
#side .op-coresplit{margin-top:4px;font-size:11.5px;color:#445}
#side .op-coresplit .cbadge{font-size:9.5px;font-weight:800;color:#fff;padding:1px 6px;border-radius:9px;vertical-align:1px}
#side .op-coresplit .cbadge.c1{background:#bd0026}#side .op-coresplit .cbadge.c2{background:#1a7a4f}
#side .op-corelegend{font-size:11px;color:#667;margin-top:2px}
#side .op-corelegend .lg{font-weight:700}#side .op-corelegend .lg.funded{color:#16a34a}#side .op-corelegend .lg.presence{color:#b7791f}
...
/* address-box ACTIONS: build+email the per-address storm report · add to the operator watchlist */
#tbar .hdr-search .ms-act{background:#1e2b46;color:#dfe8f7;border:1px solid #33456a;border-radius:6px;
  padding:5px 9px;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap}
#tbar .hdr-search .ms-act:hover{background:#274069}
#tbar .hdr-search .ms-act:disabled{opacity:.5;cursor:default}
#tbar .hdr-search .ms-actmsg{font-size:11px;color:#9fd8b0;max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#tbar .hdr-search .ms-actmsg.err{color:#ffb4a2}
...
/* -- PULL (item 10): on-demand POLYGON capture. Header button + a floating draw bar +
   a committed-pulls list. Operator draws a polygon (click vertices, close, Confirm);
   committed polygons STACK, each a distinct colour, deletable one-by-one or all. -- */
...
#pullPanel .pp-close{background:#243a5e;color:#dbe6ff;border:1px solid #365a8c;border-radius:6px;padding:6px 11px;cursor:pointer}
#pullPanel .pp-close:disabled{opacity:.5;cursor:default}
...
#pullPanel .hidden{display:none}
/* committed-pulls list: a floating panel of the polygons the operator has locked in */
#pullList{position:absolute;right:12px;top:96px;z-index:2400;width:210px;max-width:60vw;
  background:rgba(11,18,32,.96);color:#e9eef7;border:1px solid #2c6e4c;border-radius:9px;padding:9px 11px;font-size:12px;
  box-shadow:0 8px 24px rgba(0,0,0,.5)}
#pullList .pl-hd{display:flex;align-items:center;justify-content:space-between;gap:8px;border-bottom:1px solid #24406a;padding-bottom:6px;margin-bottom:6px}
#pullList .pl-hd b{font-size:11px;letter-spacing:.4px;text-transform:uppercase;color:#bdf0d6}
#pullList .pl-clear{background:#3a1620;color:#ffd0da;border:1px solid #7a2c3c;border-radius:6px;padding:3px 8px;cursor:pointer;font-size:11px}
#pullList .pl-row{display:flex;align-items:center;gap:8px;padding:4px 0}
#pullList .pl-sw{width:13px;height:13px;border-radius:3px;border:1px solid rgba(255,255,255,.55);flex:0 0 auto}
#pullList .pl-nm{flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#pullList .pl-x{background:#1e2b46;color:#ffd0da;border:1px solid #2c3c5e;border-radius:5px;width:22px;height:22px;line-height:1;cursor:pointer;flex:0 0 auto}
#pullList .pl-x:hover{background:#3a1620}
...
/* Part G: strip Leaflet's default divIcon chrome so the watched-address teardrop reads as a pin and
   not as a white box with a pin inside it. This lives in the MAIN sheet, not the watchlist panel's
...
.leaflet-tooltip.wl-pin-tip{background:#0e141f;color:#e5e7eb;border:1px solid #2a3547;border-radius:7px;
  box-shadow:0 6px 20px rgba(0,0,0,.5);padding:7px 11px;font:12px system-ui,sans-serif;
  /* Leaflet forces white-space:nowrap on tooltips, which lets a long line run past max-width and out
     of the box ("No qualifying hail on record (>= 1.00\" at the address)"). Override to normal so the
     text WRAPS inside the box, and widen a touch. min-width stops a short address making a cramped box. */
  white-space:normal;line-height:1.4;max-width:300px;min-width:150px;width:max-content}
.leaflet-tooltip.wl-pin-tip .wl-tip-addr{font-weight:700;margin-bottom:4px}
.leaflet-tooltip.wl-pin-tip .wl-tip-win{font-size:11px;font-weight:700;padding:2px 8px;border-radius:9px;
  display:inline-block;border:1px solid transparent;white-space:normal}
...
/* ── CAMPAIGN MONITOR panel (2026-08-22, Design Lock v3 §13 portfolio / §12 banner). READ-ONLY.
   Right-side overlay, same chrome family as .wx-panel; full-width on phones. ── */
.cmp-panel{position:fixed;top:0;right:0;height:100%;width:460px;max-width:97vw;z-index:2500;background:#0f1830;color:#e9eef7;
  box-shadow:-12px 0 36px rgba(0,0,0,.5);display:flex;flex-direction:column;border-left:1px solid #243352}
.cmp-panel.hidden{display:none}
.cmp-hd{display:flex;align-items:center;gap:10px;padding:13px 15px;border-bottom:1px solid #243352}
.cmp-hd b{font-size:15px;letter-spacing:.3px}.cmp-hd .cmp-sub{color:#8aa0c6;font-size:11px}
.cmp-hd .cmp-x{margin-left:auto;background:transparent;border:none;color:#9fb3d9;font-size:22px;line-height:1;cursor:pointer}
.cmp-hd .cmp-x:hover{color:#fff}
/* CONTEXT strip (Rule 11: account + as-of + window visible without scrolling) */
.cmp-ctx{padding:9px 15px;border-bottom:1px solid #243352;background:#0c1426;font-size:11px;color:#9fb3d9;display:flex;gap:14px;flex-wrap:wrap}
.cmp-ctx b{color:#cdeafe}
.cmp-modeled{margin:0;padding:8px 15px;background:#2a2410;border-bottom:1px solid #5a4a1e;color:#e6cf8a;font-size:10.5px;font-style:italic;line-height:1.4}
.cmp-modeled .cmp-badge{font-style:normal}
.cmp-badge{display:inline-block;background:#5a4a1e;color:#ffe6a0;border-radius:4px;padding:0 5px;font-size:9px;font-weight:800;letter-spacing:.4px;text-transform:uppercase;vertical-align:middle}
.cmp-body{flex:1;overflow:auto;padding:12px 15px}
.cmp-load,.cmp-err,.cmp-empty{color:#9fb3d9;font-size:13px;padding:22px 4px;text-align:center}
.cmp-err{color:#ff9a76}
.cmp-bar{display:flex;gap:8px;align-items:center;margin-bottom:12px}
.cmp-bar button{background:#243352;border:none;color:#cdeafe;border-radius:6px;padding:6px 10px;font-size:11px;font-weight:700;cursor:pointer}
.cmp-bar button:hover{background:#2f4a72}
.cmp-bar .cmp-back{background:#2f6fe0;color:#fff}
/* TOTALS strip */
.cmp-tot{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px}
.cmp-tcard{background:#13203c;border:1px solid #243352;border-radius:9px;padding:9px 11px}
.cmp-tcard .cmp-tk{font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:#8aa0c6}
.cmp-tcard .cmp-tv{font-size:18px;font-weight:800;color:#cdeafe;margin-top:2px}
.cmp-tcard .cmp-tsub{font-size:10px;color:#8aa0c6;margin-top:2px}
.cmp-tcard.wide{grid-column:1 / span 2}
/* per-campaign row */
.cmp-sec-t{font-weight:800;font-size:11px;letter-spacing:.6px;text-transform:uppercase;color:#8aa0c6;margin:4px 0 8px}
.cmp-row{background:#13203c;border:1px solid #243352;border-radius:10px;padding:10px 12px;margin-bottom:8px;cursor:pointer}
.cmp-row:hover{background:#1b2c4e;border-color:#2f4a72}
...
.cmp-pill.over-pace{background:#3a1b1b;color:#ff8a8a;box-shadow:inset 0 0 0 1px #7a2b2b}
.cmp-pill.under-pace{background:#1b2740;color:#9fb3d9;box-shadow:inset 0 0 0 1px #34496e}
.cmp-pill.too-early{background:#241b30;color:#c0a8e0;box-shadow:inset 0 0 0 1px #4a3a6a}
.cmp-flags{margin-top:8px;display:flex;flex-direction:column;gap:5px}
.cmp-flag{font-size:10.5px;border-radius:7px;padding:6px 8px;line-height:1.35}
.cmp-flag.warn{background:#2e1c12;color:#ffbf9a;box-shadow:inset 0 0 0 1px #6a3a22}
.cmp-flag.info{background:#141f36;color:#9fb8e0;box-shadow:inset 0 0 0 1px #2c3c5e}
.cmp-flag .cmp-fk{font-weight:800;text-transform:uppercase;letter-spacing:.3px;font-size:9px;margin-right:5px}
/* drill-in: rings + banner */
.cmp-rings{display:flex;flex-direction:column;gap:5px;margin-bottom:14px}
.cmp-ring{display:grid;grid-template-columns:auto 1fr auto;gap:8px;align-items:center;background:#13203c;border-radius:8px;padding:7px 10px;font-size:11px}
.cmp-ring .r-idx{font-weight:800;color:#7fc7ff;background:#0c1426;border-radius:6px;padding:2px 7px}
.cmp-ring .r-mets{color:#aebcd4;display:flex;gap:9px;flex-wrap:wrap}.cmp-ring .r-mets b{color:#e9eef7}
.cmp-bannerhd{display:flex;align-items:center;gap:8px;margin:2px 0 8px}
.cmp-bannerhd .cmp-adstr{font-size:9.5px;font-weight:800;text-transform:uppercase;letter-spacing:.4px;padding:2px 8px;border-radius:20px;background:#13203c;box-shadow:inset 0 0 0 1px #34496e;color:#cdeafe}
.cmp-assets{display:flex;flex-direction:column;gap:5px}
.cmp-asset{display:grid;grid-template-columns:74px 46px 1fr;gap:8px;align-items:center;background:#13203c;border-radius:8px;padding:7px 10px;font-size:11px}
.cmp-asset .a-ft{color:#8aa0c6;font-size:9.5px;text-transform:uppercase;letter-spacing:.4px}
.cmp-asset .a-txt{color:#dbe7fb;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.cmp-lbl{font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.3px;padding:2px 6px;border-radius:5px;text-align:center}
.cmp-lbl.BEST{background:#15301f;color:#7fe0a0}.cmp-lbl.GOOD{background:#1b2740;color:#9fb8e0}
.cmp-lbl.LOW{background:#3a1b1b;color:#ff8a8a}.cmp-lbl.LEARNING,.cmp-lbl.PENDING,.cmp-lbl.UNSPECIFIED{background:#241b30;color:#c0a8e0}
/* §11 search-terms waste list */
.cmp-stmeta{display:flex;flex-wrap:wrap;gap:10px;align-items:center;font-size:11px;color:#aebcd4;margin-bottom:6px}
.cmp-stmeta b{color:#e9eef7;font-weight:800}
.cmp-basis{font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.3px;padding:2px 7px;border-radius:20px}
.cmp-basis.proxy{background:#241b30;color:#e6cf8a;box-shadow:inset 0 0 0 1px #6a5a2b}
.cmp-basis.measured{background:#15301f;color:#7fe0a0;box-shadow:inset 0 0 0 1px #2b6a45}
.cmp-stnote{font-size:10px;color:#e6cf8a;background:#241b30;border-radius:7px;padding:6px 8px;margin-bottom:8px;line-height:1.35;box-shadow:inset 0 0 0 1px #4a3a22}
.cmp-stclasses{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px}
.cmp-cls{font-size:9.5px;color:#c0a8e0;background:#1c1730;border-radius:20px;padding:2px 8px;box-shadow:inset 0 0 0 1px #3a2f52}.cmp-cls b{color:#e0d3f4}
.cmp-sts{display:flex;flex-direction:column;gap:5px;margin-bottom:14px}
.cmp-st{background:#13203c;border-radius:8px;padding:7px 10px;font-size:11px;box-shadow:inset 0 0 0 1px #243352}
.cmp-st.waste{box-shadow:inset 0 0 0 1px #6a3a22}
.cmp-st-top{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.cmp-st .st-term{font-weight:800;color:#cdeafe;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60%}
.cmp-stflag{font-size:8.5px;font-weight:800;text-transform:uppercase;letter-spacing:.3px;padding:2px 6px;border-radius:5px}
.cmp-stflag.excl{background:#15301f;color:#7fe0a0}.cmp-stflag.added{background:#1b2740;color:#9fb8e0}
.cmp-stflag.junk{background:#3a1b1b;color:#ff8a8a}
.cmp-st-mets{display:flex;gap:10px;flex-wrap:wrap;margin-top:5px;color:#aebcd4}.cmp-st-mets b{color:#e9eef7}
.cmp-st-reason{margin-top:4px;font-size:10px;color:#8aa0c6}
/* GO-LIVE / PAUSE controls (2026-08-22, punch-list #4): the SANCTIONED operator-gated ENABLE + off-switch */
.cmp-golive{background:#13203c;border:1px solid #243352;border-radius:10px;padding:10px 12px;margin-bottom:10px}
.cmp-golive .cmp-gl-hd{display:flex;align-items:center;gap:8px;margin-bottom:6px}
.cmp-golive .cmp-gl-hd .cmp-gl-state{font-size:9.5px;font-weight:800;text-transform:uppercase;letter-spacing:.4px;padding:2px 8px;border-radius:20px}
.cmp-gl-state.live{background:#15301f;color:#7fe0a0;box-shadow:inset 0 0 0 1px #2b6a45}
.cmp-gl-state.paused{background:#1b2740;color:#9fb3d9;box-shadow:inset 0 0 0 1px #34496e}
.cmp-golive .cmp-gl-sub{font-size:10.5px;color:#8aa0c6;margin-bottom:9px;line-height:1.35}
.cmp-golive .cmp-gl-sub b{color:#e9eef7}
.cmp-gl-btn{border:none;border-radius:7px;padding:9px 14px;font-size:12px;font-weight:800;letter-spacing:.3px;cursor:pointer;width:100%}
.cmp-gl-btn.go{background:#c0392b;color:#fff}.cmp-gl-btn.go:hover{background:#d84636}
.cmp-gl-btn.pause{background:#243352;color:#cdeafe}.cmp-gl-btn.pause:hover{background:#2f4a72}
.cmp-gl-btn:disabled{opacity:.5;cursor:default}
.cmp-gl-msg{margin-top:8px;font-size:10.5px;border-radius:7px;padding:7px 9px;line-height:1.4}
.cmp-gl-msg.ok{background:#15301f;color:#7fe0a0;box-shadow:inset 0 0 0 1px #2b6a45}
.cmp-gl-msg.err{background:#2e1c12;color:#ffbf9a;box-shadow:inset 0 0 0 1px #6a3a22}
/* CONFIRM dialog — spends real money; a deliberate, unmissable gate */
.cmp-confirm-bk{position:fixed;inset:0;z-index:2600;background:rgba(0,0,0,.55);display:flex;align-items:center;justify-content:center;padding:16px}
.cmp-confirm{background:#0f1830;border:1px solid #34496e;border-radius:12px;max-width:420px;width:100%;padding:18px 18px 16px;box-shadow:0 12px 40px rgba(0,0,0,.55);color:#e9eef7}
.cmp-confirm h3{margin:0 0 10px;font-size:15px;font-weight:800}
.cmp-confirm h3.go{color:#ff8a7a}.cmp-confirm h3.pause{color:#9fb8e0}
.cmp-confirm .cmp-cf-rows{background:#13203c;border-radius:9px;padding:10px 12px;margin-bottom:12px;font-size:12px}
.cmp-confirm .cmp-cf-rows .r{display:flex;justify-content:space-between;gap:10px;padding:3px 0}
.cmp-confirm .cmp-cf-rows .r span{color:#8aa0c6}.cmp-confirm .cmp-cf-rows .r b{color:#e9eef7;font-weight:800;text-align:right}
.cmp-confirm .cmp-cf-warn{background:#2e1c12;color:#ffcbb0;border-radius:9px;padding:9px 11px;font-size:11.5px;line-height:1.45;margin-bottom:14px;box-shadow:inset 0 0 0 1px #6a3a22}
.cmp-confirm .cmp-cf-warn b{color:#ffd7a0}
.cmp-confirm .cmp-cf-btns{display:flex;gap:9px}
.cmp-confirm .cmp-cf-btns button{flex:1;border:none;border-radius:8px;padding:10px;font-size:12px;font-weight:800;cursor:pointer}
.cmp-confirm .cmp-cf-cancel{background:#243352;color:#cdeafe}.cmp-confirm .cmp-cf-cancel:hover{background:#2f4a72}
...
.swath-hover-tip{background:#0d1526;color:#eaf1ff;border:1px solid #2b3d5f;border-radius:6px;padding:5px 8px;font-size:11px;line-height:1.35;font-weight:600;box-shadow:0 2px 10px rgba(0,0,0,.4)}
.swath-hover-tip b{color:#7fb2ff}
.swath-hover-tip:before{border-top-color:#0d1526}
/* pointer-events:auto so the hover title actually appears. Both badges are small bottom-corner
   chips, so the map area they intercept is negligible; without this the title is dead markup. */
.swathbadge{position:absolute;left:10px;bottom:26px;z-index:640;pointer-events:auto;cursor:help;font:600 11px/1.3 system-ui,-apple-system,sans-serif;padding:5px 9px;border-radius:7px;box-shadow:0 2px 8px rgba(0,0,0,.35);letter-spacing:.02em}
...
/* PER-PERIL glance badges (truth pass) — named wind/tornado corner labels, stacked ABOVE the hail badge */
.perilbadges{position:absolute;left:10px;bottom:46px;z-index:641;display:flex;flex-direction:column;gap:4px;pointer-events:none}
.perilbadge{font:800 10px/1.3 system-ui,-apple-system,sans-serif;letter-spacing:.05em;text-transform:uppercase;padding:4px 9px;border-radius:7px;box-shadow:0 2px 8px rgba(0,0,0,.35)}
.perilbadge.tornado{background:rgba(58,14,14,.92);color:#ffd2cd;border:1px solid #7a2a20}
.perilbadge.wind{background:rgba(8,32,72,.92);color:#cfe1ff;border:1px solid #24487f}
.swath-hover-tip .oom{color:#9fb0c8;font-weight:700}
...
.chaseoom{position:absolute;right:10px;bottom:26px;z-index:640;pointer-events:none;font:800 10.5px/1.3 system-ui,-apple-system,sans-serif;
  ...
  border:1px dashed #c98a2e;box-shadow:0 2px 8px rgba(0,0,0,.35)}
.chaseoom span{display:block;text-transform:none;letter-spacing:.02em;font-weight:600;color:#9fb0c6;margin-top:1px}
/* Chase hull label — sits ON the amber dash-dot hull and states what the shape is NOT. Amber ties
   it to the hull stroke; the market geofence never uses a warm colour. */
/* Muted at REST, full contrast on HOVER (2026-07-20). At rest the amber-on-dark label was hard
   to read over a light basemap and competed with the swath it annotates; it now sits back as a
   small dim chip and comes forward when pointed at - the tooltip-expand pattern used elsewhere.
   pointer-events:auto so the hover can land. */
.chasehull-lbl{white-space:nowrap;font:700 9px/1.2 system-ui,-apple-system,sans-serif;letter-spacing:.02em;
  color:#c9a06a;background:rgba(18,15,10,.55);border:1px solid rgba(201,138,46,.45);border-radius:5px;
  padding:1px 5px;transform:translateX(-50%);pointer-events:auto;cursor:default;opacity:.72;
  transition:opacity .12s,color .12s,background-color .12s,font-size .12s}
.chasehull-lbl:hover{font-size:11px;opacity:1;color:#ffdca8;background:rgba(24,20,12,.95);
  border-color:#c98a2e;text-shadow:0 1px 2px rgba(0,0,0,.95);box-shadow:0 2px 10px rgba(0,0,0,.5)}
/* Why-line inside a visibility-floor marker tooltip: explains the dot instead of a swath. */
.whyline{display:block;margin-top:3px;font-weight:600;color:#9fb0c6;font-size:10px;line-height:1.3}
/* ── ON-MAP TIER LEGEND (2026-07-20) — collapsible, same vocabulary as the calendar popup so one
   legend teaches both surfaces.
   POSITIONED BY LEAFLET, not by us. It first shipped as position:absolute;top:78px and landed
   ON TOP of the hover readout, which is an L.control({position:'topleft'}) — the operator's
   screenshot showed the readout covering the legend's own header. It is now a real Leaflet
   control in the same top-left stack, so Leaflet flows it below zoom + readout and keeps that
   true at any viewport size instead of us guessing a pixel offset. ── */
#tierLegend{font:600 10.5px/1.35 system-ui,-apple-system,sans-serif;
  background:rgba(13,21,38,.93);color:#cfd9ea;border:1px solid #2c3c5e;border-radius:8px;
  box-shadow:0 3px 12px rgba(0,0,0,.45);pointer-events:auto;max-width:210px}
#tierLegend .tl-h{display:flex;align-items:center;gap:6px;padding:5px 9px;cursor:pointer;
  font-weight:800;letter-spacing:.05em;text-transform:uppercase;font-size:9.5px;color:#9fb3d9}
#tierLegend .tl-h .tl-caret{margin-left:auto;transition:transform .15s}
#tierLegend.closed .tl-h .tl-caret{transform:rotate(-90deg)}
#tierLegend.closed .tl-b{display:none}
#tierLegend .tl-b{padding:2px 9px 8px}
#tierLegend .tl-sec{margin-top:5px;font-size:9px;letter-spacing:.05em;text-transform:uppercase;color:#7f8fab;font-weight:800}
#tierLegend .tl-row{display:flex;align-items:center;gap:7px;margin-top:3px}
#tierLegend .tl-sw{width:18px;height:10px;border-radius:2px;flex:0 0 auto}
#tierLegend .tl-ln{width:18px;height:0;flex:0 0 auto;border-top-width:2px}
...
/* dual-core label reconcile: FUNDED = combined damaged>0 (solid, green halo); PRESENCE-ONLY =
   ad-eligible but damaged==0 (hollow, amber dashed). Tier color stays as the fill for funded. */
.rankbadge.funded{border-color:#28d17c;box-shadow:0 0 0 2px rgba(40,209,124,.55),0 1px 4px rgba(0,0,0,.5)}
.rankbadge.presence{background:transparent;color:#5a3a1a;border:1.5px dashed #f0a020;box-shadow:none}
.rankbadge.presence .rs{border-left-color:rgba(90,58,26,.4)}
.cprov{font:800 8px/1 system-ui,sans-serif;margin-left:3px;padding:1px 4px;border-radius:8px;color:#fff}
.cprov.c1{background:#bd0026}.cprov.c2{background:#1a7a4f}.cprov.both{background:#7a4fd6}
...
.sd-promo.provisional{border-left-color:#c77d0a;background:#fff7ec}
.sd-promo .prov{display:inline-block;background:#c77d0a;color:#fff;font-size:9.5px;font-weight:800;letter-spacing:.4px;padding:2px 7px;border-radius:9px;margin-right:6px;vertical-align:1px}
.sd-promo .mc{display:block;margin-top:4px;font-weight:800;color:#fff;background:#b3341a;padding:4px 8px;border-radius:6px}
...
.sd-verdict.parcelbasis{border-left-color:#7a4fd6}
.sd-peril{display:inline-block;background:#6a2fb5;color:#fff;font-size:9px;font-weight:800;letter-spacing:.5px;padding:1px 6px;border-radius:9px;vertical-align:1px}
.sd-basis{display:inline-block;background:#efe7fb;color:#5a2fa0;font-size:9px;font-weight:800;letter-spacing:.4px;padding:1px 6px;border-radius:9px}
.tchip{position:absolute;left:10px;top:10px;z-index:641;display:flex;flex-direction:column;gap:4px;pointer-events:none}
.tchip span{font:800 10px/1.3 system-ui,sans-serif;padding:3px 8px;border-radius:9px;box-shadow:0 2px 8px rgba(0,0,0,.3);letter-spacing:.02em}
.tchip .fund{background:#1a7a4f;color:#fff}.tchip .nofund{background:#8a6d3b;color:#fff}
.tchip .miss{background:#b3341a;color:#fff}.tchip .pinc{background:#c77d0a;color:#fff}
...
SHELL_HEAD = """  <div id="tbar">
    ...
    <div class="hdr-search">
      ...
      <button id="opGear" class="ms-act" type="button" title="Enter your operator code (one-time, this device) to bypass the INSTANT PRICE rate caps.">&#9881;</button>
    ...
    <button id="pullBtn" title="Draw a polygon to pull on-demand storm data for that area (stackable; multiple pulls)">&#10515; Pull</button>
    ...
    <div id="tempCard" class="hidden">
      ...
      <div class="tc-body">
        ...
        <div class="tc-btns">
          ...
          <button id="tcClaim" type="button" title="How close was this address to 1.00&quot; and 0.75&quot; hail in the last 12 months (public storm record). For the denied/underpaid-claim conversation.">&#128269; Claim Check</button>
...
// POLYGON PULL (Phase 2): store / delete a per-date operator-drawn capture polygon. Write token
// held server-side in the Netlify functions; this bundle carries NO credential. LIST rides the
// read /api/storm -> /query whitelist (name "storm_pull_areas") via pquery().
const PULL_STORE_API  = CFG.pullStoreApi  || API.replace(/\\/api\\/storm$/, "/api/storm-pull-store");
const PULL_DELETE_API = CFG.pullDeleteApi || API.replace(/\\/api\\/storm$/, "/api/storm-pull-delete");
...
// ── LIVE-SWATH HOVER READOUT (Part C): hail size / scan-time-of-max / scan-persistence ──
// The live 30-min watch writes 7-wide cells [lat,lng,size,intensity,dur,flag,maxTs]. Hover
// hit-tests the underlying cell GRID at the cursor lat/lng, so it reads IDENTICALLY in both
// render modes — Cells (per-cell rects) and Smooth (the imageOverlay raster is dumb). Gated to
// 7-wide cells so it only lights up on the live provisional swath, not historical dates.
// PER-PERIL hover index (truth pass, 2026-07-13): EVERY rendered swath is point-identifiable, not
// just the live hail one. Works at cellW>=3 so HISTORICAL swaths (6-wide hail, 3-wide wind/tornado)
// and the out-of-market chase swath all gain hover; the richer size/scan-time/persistence line only
// shows on 7-wide LIVE hail cells. `_HOVER.layers` is priority-ordered (topmost/most-specific first);
// the mousemove reads the live array so re-calling initSwathHover per date refreshes it in place.
let _HOVER = {layers:[], tip:null, wired:false};
// Current chase-hull boundary + its OWN tooltip. Set by addChaseSwathLayer, read by the hover
// handler so the BOUNDARY answers for itself instead of leaking the chase CELL value underneath.
let CHASE_HULL_PTS = null;
const CHASE_HULL_TIP = '<b>chase activity extent</b><br>'
  + '<span class="oom">not the market boundary \u00b7 no hail value here</span>';
function _hoverKey(la, lo, st){ return (Math.round(la/st)*st).toFixed(3)+","+(Math.round(lo/st)*st).toFixed(3); }
function _fmtScanTs(ts){
  const m=/^(\\d{4})(\\d{2})(\\d{2})-(\\d{2})Z?$/.exec(String(ts||"")); if(!m) return String(ts||"\\u2014");
  try{ return new Intl.DateTimeFormat("en-US",{month:"numeric",day:"numeric",hour:"numeric",hour12:true,
       timeZone:"America/Chicago"}).format(new Date(Date.UTC(+m[1],+m[2]-1,+m[3],+m[4]))); }catch(e){ return String(ts); }
}
function _hoverGrid(cells, peril){                                  // one peril's cell grid, or null
  if(!cells || !cells.length || !cells[0] || cells[0].length<3) return null;
  const lats=[...new Set(cells.map(c=>+c[0]))].sort((a,b)=>a-b);
  let st=0.01; for(let i=1;i<lats.length;i++){ const d=lats[i]-lats[i-1]; if(d>1e-6 && d<st) st=d; }
  st=st||0.01;
  const m=new Map(); for(const c of cells) m.set(_hoverKey(+c[0],+c[1],st), c);
  return { step:st, map:m, peril, live:(cells[0].length>=7) };
}
function _hoverTip(g, c){
  const v=(+c[2]||0).toFixed(2);
  if(g.peril==="hail" || g.peril==="chase"){
    let s='<b>'+v+'\\u2033 hail</b>'+(g.peril==="chase"?' <span class="oom">out of market \\u00b7 chase</span>':'');
    if(g.live && c.length>=7) s+='<br>max @ '+_fmtScanTs(c[6])+'<br>'+(+c[4]||0)+' scan'+((+c[4]||0)===1?'':'s')+' with hail';
    return s;
  }
  if(g.peril==="tornado") return '<b>tornado corridor</b><br>rotation index '+v;
  if(g.peril==="wind")    return '<b>wind swath</b><br>gust index '+v;
  return '<b>'+v+'</b>';
}
function initSwathHover(D){
  _HOVER.layers = [];
  const add=(cells,peril)=>{ const g=_hoverGrid(cells,peril); if(g) _HOVER.layers.push(g); };
  add((D&&D.swath_cells)||[], "hail");                             // priority: in-market hail first
  if(D&&D.tornado) add(D.tornado.swath_cells||[], "tornado");
  if(D&&D.wind)    add(D.wind.swath_cells||[], "wind");
  if(D&&D.chase)   add((D.chase.swath_cells)||[], "chase");        // out-of-market chase last
  if(!TMAP || _HOVER.wired) return;
  _HOVER.wired=true;
  _HOVER.tip=L.tooltip({sticky:true, direction:"top", opacity:0.95, className:"swath-hover-tip"});
  TMAP.on("mousemove", e=>{
    // CHASE-HULL BOUNDARY WINS (2026-07-20). The hull polygon is interactive:false and emits no
    // tooltip of its own, so hovering the amber dash-dot line fell through to the per-cell index
    // below and popped the CHASE CELL under the cursor. The value changed as you slid along the
    // line, which reads as if the LINE encoded hail size. It does not - it is only the extent of
    // chase activity. Within 7px of that boundary the line speaks for itself: no hail number.
    if(CHASE_HULL_PTS && CHASE_HULL_PTS.length>2){
      const p=TMAP.latLngToLayerPoint(e.latlng); let near=false;
      for(let i=0;i<CHASE_HULL_PTS.length && !near;i++){
        const a=TMAP.latLngToLayerPoint(L.latLng(CHASE_HULL_PTS[i])),
              q=TMAP.latLngToLayerPoint(L.latLng(CHASE_HULL_PTS[(i+1)%CHASE_HULL_PTS.length]));
        const dx=q.x-a.x, dy=q.y-a.y, LL=dx*dx+dy*dy;
        let t=LL?(((p.x-a.x)*dx+(p.y-a.y)*dy)/LL):0; t=Math.max(0,Math.min(1,t));
        const ex=a.x+t*dx-p.x, ey=a.y+t*dy-p.y;
        if(ex*ex+ey*ey <= 49) near=true;
      }
      if(near){ _HOVER.tip.setLatLng(e.latlng).setContent(CHASE_HULL_TIP);
        TMAP.openTooltip(_HOVER.tip); return; }
    }
    for(const g of _HOVER.layers){
      const c=g.map.get(_hoverKey(e.latlng.lat, e.latlng.lng, g.step));
      if(c){ _HOVER.tip.setLatLng(e.latlng).setContent(_hoverTip(g,c)); TMAP.openTooltip(_HOVER.tip); return; }
    }
    TMAP.closeTooltip(_HOVER.tip);
  });
  TMAP.on("mouseout", ()=>{ if(_HOVER.tip) TMAP.closeTooltip(_HOVER.tip); });
  // MOBILE TAP FIX (2026-08-23): on touch, a tap fires mouseover->mouseout back-to-back, so the hover
  // tooltip flashed and vanished — worst on the OUT-OF-MARKET chase swath (in-market taps are covered by
  // the meshAt popup, which persists; chase is not). On tap, when meshAt has no in-market cell, resolve
  // the hull/chase cell and open a PERSISTENT popup (close button) that survives mouseout. Desktop click too.
  TMAP.on("click", e=>{
    if(window.meshAt && window.meshAt(e.latlng.lat, e.latlng.lng)) return;   // in-market handled by meshAt popup
    let html=null;
    if(CHASE_HULL_PTS && CHASE_HULL_PTS.length>2){
      const p=TMAP.latLngToLayerPoint(e.latlng); let near=false;
      for(let i=0;i<CHASE_HULL_PTS.length && !near;i++){
        ...
        const dx=q.x-a.x, dy=q.y-a.y, LL=dx*dx+dy*dy;
        let t=LL?(((p.x-a.x)*dx+(p.y-a.y)*dy)/LL):0; t=Math.max(0,Math.min(1,t));
        const ex=a.x+t*dx-p.x, ey=a.y+t*dy-p.y; if(ex*ex+ey*ey<=49) near=true;
      }
      if(near) html=CHASE_HULL_TIP;
    }
    if(!html){ for(const g of _HOVER.layers){ const c=g.map.get(_hoverKey(e.latlng.lat, e.latlng.lng, g.step)); if(c){ html=_hoverTip(g,c); break; } } }
    if(html) L.popup({closeButton:true, autoClose:true, className:"swath-hover-tip"})
              .setLatLng(e.latlng).setContent(html).openOn(TMAP);
  });
}

// ── LIVE-SWATH GLANCE BADGE: the corner truth (companion to the point-truth hover). ──
// Sub-gate:  "sub-gate · max 0.84" · 0 fundable cells · cumulative since 17Z"  (grey-blue, muted)
// Gated:     "max X" · N gated cells"                                          (warm/red)
// Values are LIVE from the swath payload (size c[2] in inches; gate 1.125"; earliest scan c[6]).
let _SWATHBADGE = null;
function _fmtSinceHr(ts){ const m=/-(\\d{2})Z?$/.exec(String(ts||"")); return m ? m[1]+"Z" : null; }
function updateSwathBadge(cells, cls){
  const host=document.getElementById("map"); if(!host) return;
  if(!_SWATHBADGE){ _SWATHBADGE=document.createElement("div"); _SWATHBADGE.className="swathbadge"; host.appendChild(_SWATHBADGE); }
  const b=_SWATHBADGE;
  if(!cells || !cells.length){ b.style.display="none"; return; }
  let mx=0, gated=0, since=null;
  for(const c of cells){ const s=+c[2]||0; if(s>mx) mx=s; if(s>=1.125) gated++;
    if(c.length>=7 && c[6] && (since===null || c[6]<since)) since=c[6]; }
  const live=(cells[0].length>=7);
  // LEGIBILITY (2026-07-20): lead with the TIER WORD on both branches. The gated badge used to
  // open with a bare number, so the fundable tier was the only one the map never named — the
  // operator had to infer it from the colour. Hover expands every abbreviation (title works
  // because .swathbadge now takes pointer events).
  //
  // THE TIER WORD COMES FROM classify.py (D.classification.cls), NOT from a local recount of
  // gated cells. Caught in review on 2026-07-10: that date has 10 gated cells but classifies
  // SUB_GATE, so a gated>0 recount printed "FUNDABLE" on the badge while the floor marker on the
  // SAME date said "Sub-gate hail". Two surfaces, two answers, and the badge's was not the
  // authoritative one. The gated count still drives the COLOUR (did any cell reach the gate);
  // only classify.py names the tier. No classification (older payload) -> fall back to the
  // pre-existing count-based wording rather than assert a tier we were not given.
  const _isSub=(cls===CLS.SUB), _isFund=(cls===CLS.FUND);
  const _tier=_isFund?'FUNDABLE':(_isSub?'SUB-GATE':null);
  if(gated>0){ b.className="swathbadge gated";
    b.textContent=(_tier?_tier+' \\u00b7 ':'')+'max '+mx.toFixed(2)+'\\u2033 \\u00b7 '+gated+' gated cell'+(gated===1?'':'s');
    b.title='FUNDABLE — in-market hail at or above the 1.125\\u2033 funding gate.\\n'
      +'max = largest hail size measured anywhere in this swath.\\n'
      +'gated cells = 0.005\\u00b0 radar cells at or above the gate (the fundable footprint).'; }
  else { b.className="swathbadge subgate";
    b.textContent=(_tier||'SUB-GATE')+' \\u00b7 max '+mx.toFixed(2)+'\\u2033 \\u00b7 0 fundable cells'
      + ((live && _fmtSinceHr(since)) ? ' \\u00b7 cumulative since '+_fmtSinceHr(since) : '');
    b.title='SUB-GATE — in-market hail BELOW the 1.125\\u2033 funding gate. Real hail, '
      +'not fundable: no cell reached the gate.\\n'
      +'max = largest hail size measured anywhere in this swath.\\n'
      +'cumulative since = earliest scan hour merged into this footprint (UTC).'; }
  b.style.display="";
}

// PER-PERIL glance badges (truth pass, 2026-07-13): wind + tornado swaths get a NAMED corner label
// (peril · max · cell count), stacked above the hail badge, so no rendered raster is nameless. Chase
// keeps its own OUT OF MARKET badge (addChaseSwathLayer). Hail keeps updateSwathBadge above.
let _PERILBADGEBOX = null;
function updatePerilBadges(D){
  const host=document.getElementById("map"); if(!host) return;
  if(!_PERILBADGEBOX){ _PERILBADGEBOX=document.createElement("div"); _PERILBADGEBOX.className="perilbadges"; host.appendChild(_PERILBADGEBOX); }
  const box=_PERILBADGEBOX; box.innerHTML="";
  const nums=cells=>{ let mx=0; for(const c of (cells||[])){ const s=+c[2]||0; if(s>mx) mx=s; } return {mx, n:(cells||[]).length}; };
  const add=(cls,html)=>{ const b=document.createElement("div"); b.className="perilbadge "+cls; b.innerHTML=html; box.appendChild(b); };
  if(D && D.tornado && D.tornado.swath_cells && D.tornado.swath_cells.length){ const t=nums(D.tornado.swath_cells);
    add("tornado","TORNADO \\u00b7 rot "+t.mx.toFixed(1)+" \\u00b7 "+t.n+" cell"+(t.n===1?'':'s')); }
  if(D && D.wind && D.wind.swath_cells && D.wind.swath_cells.length){ const w=nums(D.wind.swath_cells);
    add("wind","WIND \\u00b7 max "+w.mx.toFixed(1)+" \\u00b7 "+w.n+" cell"+(w.n===1?'':'s')); }
...
// Core-3 (2026-07-12): the deferred portal CHIPS, read from the additive storm_date_layers fields —
// tornado gate verdict (fundable / not + reason), the miss-recovery review flag, and the
// pricing-incomplete flag. Renders a small stack on the map; absent fields -> no chip.
function tornadoChips(rows){
  const tor = (rows||[]).find(r => r.peril === "tornado");
  return { tornado_fundable: tor ? !!tor.fundable : null,
           tornado_reason: tor ? (tor.not_promotable_reason || "") : "",
           miss_recovery: (rows||[]).some(r => r.miss_recovery),
           passes_incomplete: (rows||[]).some(r => r.passes_incomplete) };
}
function renderTornadoChips(D){
  const host=document.getElementById("map"); if(!host) return;
  ...
  if(c.passes_incomplete) parts.push("<span class='pinc'>\\u26a0 PRICING INCOMPLETE</span>");
  if(!parts.length) return;
  const el=document.createElement("div"); el.id="tchips"; el.className="tchip"; el.innerHTML=parts.join("");
  host.appendChild(el);
}
...
function assemble(date, rows, geo){
  ...
  const D = {
    ...
    // dual-core (2026-07-05): combined damage core = intensity>=8.5 (Core 1) OR size>=2.0"+contiguity
    // (Core 2), parcels credited once. Graceful: absent (older cache) -> rows hidden, verdict unchanged.
    core2_count: (h && h.core2_count != null) ? +h.core2_count : null,
    combined_core_count: (h && h.combined_core_count != null) ? +h.combined_core_count : null,
    core2_size_floor: (h && h.core2_size_floor != null) ? +h.core2_size_floor : null,
    core2_min_contig: (h && h.core2_min_contig != null) ? +h.core2_min_contig : null,
  };
  D.chips = tornadoChips(rows);                                 // Core-3 portal chips (from proxy fields)
...
// Open the Spend Dial from anywhere (the sub-gate "Work this storm" button). Mobile = bottom sheet,
// desktop = the #tbar2 float toggle. A sub-gate storm is a real in-market storm the auto gate declined to
// FUND automatically; the manual solve->dial->promote->approve path works on it (the funding gate only stops
// automation), so this just surfaces the dial the operator otherwise had to know to open. 2026-09-18.
window.tempestOpenSpendDial = function(){
  var m = document.querySelector('#mtoolbar [data-sheet="spend"]');
  if(m){ m.click(); return; }
  var d = document.querySelector('#tbar2 [data-fb="spend"]');
  if(d){ d.click(); }
};
...
function buildOperatorPanel(D, forecast){
  ...
  // SUB-GATE "work it anyway" button (2026-09-18): a real in-market storm the auto gate declined to FUND is
  // still workable by hand (the manual dial->promote->approve path bypasses the automation-only gate). One tap
  // opens the Spend Dial on this date; it solves at the 1.25" floor by default, so the marginal cells show as
  // targets. Empty-queue days are exactly when the operator mines these for roofs.
  if(hailCls===CLS.SUB){
    h+='<button onclick="tempestOpenSpendDial()" style="margin:10px 0 3px;padding:10px 12px;width:100%;'
      +'background:#2f6fe0;color:#fff;border:none;border-radius:8px;font-weight:800;font-size:13px;cursor:pointer">'
      +'\\u26a1 Work this storm \\u2014 open Spend Dial</button>'
      +'<div style="font-size:10.5px;color:#8a6d3b;margin:0 0 4px;line-height:1.35">Below the auto funding gate '
      +'\\u2014 no campaign launched itself, but you can dial and launch it by hand when you want the roofs.</div>';
  }
  ...
  if(D.payout){
    let nranked=0, nmuted=0, nfund=0, npres=0;
    ["hail","wind","tornado"].forEach(pk=>{ const ps=D.payout[pk]; if(!ps) return;
      Object.keys(ps).forEach(k=>{ nranked++; if(!ps[k].floor_pass) nmuted++;
        if((ps[k].damaged_homes||0)>0) nfund++; else npres++; }); });
    if(nranked){ h+='<div class="op-row"><b>Ad targets:</b> '+nranked+
      ' <span class="muted">('+(nranked-nmuted)+' above floor \\u00b7 '+nmuted+' muted)</span></div>';
      // legend: funded core (solid green) vs presence-only (hollow amber) — the label reconcile
      h+='<div class="op-row op-corelegend"><span class="lg funded">\\u25cf funded core</span> '+nfund+
        ' &nbsp;\\u00b7&nbsp; <span class="lg presence">\\u25cb presence only</span> '+npres+'</div>';
    }
  }
  // ── DUAL-CORE damage core (2026-07-05): combined = intensity\\u22658.5 (Core 1) OR size\\u22652.0"
  //    +contiguity (Core 2), credited once. Presence-only banner mirrors the Solve verdict. ──
  const hailPeril = perils.some(p=>p.k==="hail");
  if(hailPeril && D.combined_core_count!=null){
    const c1=D.list_core_count||0, c2=D.core2_count||0, comb=D.combined_core_count;
    const flr=(D.core2_size_floor!=null?D.core2_size_floor:2.0).toFixed(1);
    if(comb>0){
      h+='<div class="op-row op-core2"><b>Damage core (combined):</b> '+comb.toLocaleString()+
        ' parcels <span class="muted">funds search spend</span>'+
        '<div class="op-coresplit"><span class="cbadge c1">C1 \\u22658.5 intensity</span> '+c1.toLocaleString()+
        ' &nbsp;\\u00b7&nbsp; <span class="cbadge c2">C2 \\u2265'+flr+'" size</span> '+c2.toLocaleString()+
        ' <span class="muted">(overlap credited once)</span></div></div>';
    } else {
      h+='<div class="op-verdict none op-nocore">Presence only \\u2014 no damage core '+
        '(intensity \\u22658.5 or \\u2265'+flr+'" size); campaign = $0</div>';
    }
  }
  if(D.list_core_count!=null) h+='<div class="op-row"><b>List core (\\u22658.5):</b> '+
    D.list_core_count.toLocaleString()+' parcels <span class="muted">door/call \\u00b7 Core 1 only</span></div>';
...
function paintRankedTargets(D){
  ...
  const markers=[]; let scored=0, muted=0, funded=0, presence=0;
  perils.forEach(([pk, circles])=>{
    ...
    circles.forEach((c, i)=>{
      ...
      if((s.damaged_homes||0)>0) funded++; else presence++;
      // dual-core label reconcile: combined damaged>0 => FUNDED (solid green); ad-eligible but
      // damaged==0 => PRESENCE-ONLY (hollow amber). Provenance chip = which core(s) fired.
      const dmg = s.damaged_homes||0, dc1 = s.core1_damaged||0, dc2 = s.core2_damaged||0;
      const status = dmg>0 ? "funded" : "presence";
      let prov="", pc=""; if(dmg>0){ if(dc1>0&&dc2>0){prov="C1+C2";pc="both";} else if(dc2>0){prov="C2";pc="c2";} else {prov="C1";pc="c1";} }
      const html='<div class="rankbadge t'+(c.tier||"").slice(1)+' '+status+(s.floor_pass?"":" muted")+'">'
        +'<b>#'+s.rank+'</b><span class="rs">'+Math.round(s.payout_score)+'</span>'
        +(prov?'<span class="cprov '+pc+'">'+prov+'</span>':'')+'</div>';
  ...
  if(cs && scored) cs.textContent = scored+" ranked targets \\u00b7 "+funded+" funded core \\u00b7 "+
    presence+" presence-only"+(muted?(" \\u00b7 "+muted+" below floor"):"");
}
...
const SD_COMMANDS = ["solve", "active-promotion", "promote-solve", "send-promoted",
                     "address-report", "hail-proximity", "watchlist-add",
                     ...
                     "resolve",
                     // CAMPAIGN MONITOR (2026-08-22): READ-ONLY portfolio rollup + per-campaign detail + banner (Design Lock v3 §13/§12)
                     "campaign-status",
                     ...
                     "update-live-geo",
                     // LEVER E (2026-09-12): reshape/fit-to-win-cheap. preview = READ-ONLY proposals
                     // (recenter/re-radius/bid to win the auction cheaper, writes nothing); apply = the
                     // gated live push (persists map overrides -> update-live-geo). Both are /api/spend-
                     // <action> redirect-covered (netlify-proxy/netlify.toml).
                     "lever-e-preview", "lever-e-apply"];
async function sdApi(action, body){
  ...
  try{
    ...
    // storm call would break the other functions' Content-Type-only preflight). The OPERATOR BYPASS token
    // (if the operator entered one on this device) rides the same lane -> exempts the caps server-side.
    if(action==="packet" || action==="packet-status"){ _hdrs["X-KCC-Portal"]="kcc-storm-portal";
      try{ const _oc=localStorage.getItem("kcc_operator_code"); if(_oc) _hdrs["X-KCC-Operator"]=_oc; }catch(e){} }
...
//    OPERATOR-FACING ONLY. The report is emailed to the operator; the watchlist digests to the
//    operator. No homeowner is contacted by either path in v1.
function msActMsg(txt, err){
...
let TC_LAST=null;                    // retained resolve state (2026-08-01): the "Card" relaunch button re-opens from this
...
function showTempCard(addr, lat, lng, opts){
  ...
  TC_LAST={addr:addr, lat:lat, lng:lng, opts:opts};   // retained for the map-pin popup card icon (no re-resolve)
  ...
  if(!c.dataset.wired){ c.dataset.wired="1";
    ...
    // CLAIM CHECK (2026-09-16): nearest 1.00"/0.75" hail to this address in the last 12 months (READ-ONLY).
    document.getElementById("tcClaim").addEventListener("click", async()=>{
      const ll=TC_LAST||{};
      tcMsg("Checking hail near this address (last 12 mo)\\u2026");
      try{
        const r=await sdApi("hail-proximity",{address:TC_ADDR, lat:ll.lat, lon:ll.lng});
        if(r&&r.ok===false){ tcMsg(r.error||"Check failed.",true); return; }
        const cg=r.nearest&&r.nearest["1.00"];              // nearest claim-grade (>=1.0") hail
        const atpt=(r.at_point_in||0), mx5=(r.max_within_5mi_in||0);
        let read;
        if(cg && cg.distance_mi<=5) read="Claim-grade (\\u22651\\u2033) hail was "+cg.distance_mi+" mi away \\u2014 this roof may have caught damage; worth an inspection.";
        else if(cg) read="Nearest claim-grade (\\u22651\\u2033) hail was "+cg.distance_mi+" mi away \\u2014 likely too far; weak case here.";
        else read="No claim-grade (\\u22651\\u2033) hail found nearby \\u2014 weak; any neighbor roofs are likely from another storm.";
        tcMsg("Last 12 mo \\u2014 at this address: "+atpt+"\\u2033 \\u00b7 max within ~5 mi: "+mx5+"\\u2033 \\u00b7 nearest \\u22651\\u2033: "
              +(cg? (cg.distance_mi+" mi ("+cg.size_in+"\\u2033, "+cg.date+")") : "none")+". "+read);
      }catch(e){ tcMsg("Failed \\u2014 "+e.message,true); }
    });
  }
...
const WL_STR_FIELDS = [
  ...
  ["carrier","Insurance carrier"],["policy_number","Policy #"],["claim_number","Claim #"],
  ["phone","Phone"],["email","Email"],["roof_type","Roof type"],["marker_color","Marker color"],
  ["note","Notes"]];
...
function wlInject(){
  ...
  #wlPop .wl-hgt-noread{color:#94a3b8;font-style:italic}
  ...
  /* Part F: a full-width bottom EDGE handle for drag-to-lengthen. The panel already carried CSS
     resize:both, but that grip is a ~16px corner square — usable with a mouse, poor for a touch
     drag, and it cannot lengthen without also changing width. This bar spans the panel so height is
     draggable on its own. It stops short of the corner (right:14px) so the two never fight for the
     same pixels. Applies to BOTH views: #wlPop is ONE container that swaps the list for the address
     detail, so the list box and the popup inherit this from the same element. */
  #wlPop .wl-vgrip{position:absolute;left:0;right:14px;bottom:0;height:10px;cursor:ns-resize;
    touch-action:none;background:transparent}
  #wlPop .wl-vgrip::after{content:"";position:absolute;left:50%;transform:translateX(-50%);
    bottom:3px;width:44px;height:3px;border-radius:2px;background:#33415a}
  #wlPop .wl-vgrip:hover::after{background:#60a5fa}
  /* D3 claim-window chip. Three tones, one definition -- the tone name arrives from the API
     (storm.claim_window) and the UI only colours it, so this can never disagree with the address
     panel. Colours carry a text label too: tone alone must not be the only carrier of meaning. */
  #wlPop .wl-cw{display:inline-block;padding:2px 7px;border-radius:10px;font-size:11px;
    font-weight:700;white-space:nowrap;border:1px solid transparent}
  #wlPop .wl-cw-green{background:#062b18;color:#4ade80;border-color:#14532d}
  #wlPop .wl-cw-amber{background:#3a1206;color:#fb923c;border-color:#7c2d12}
  #wlPop .wl-cw-grey {background:#1a212e;color:#94a3b8;border-color:#2a3547}
  /* D4 filter bar */
  #wlPop .wl-filters{flex-shrink:0;padding:8px 12px 6px;border-bottom:1px solid #263041;background:#111826}
  #wlPop .wl-frow{display:flex;gap:6px;margin-bottom:6px;flex-wrap:wrap}
  #wlPop .wl-in{flex:1;min-width:96px;background:#0b111c;color:#e5e7eb;border:1px solid #2a3547;
  ...
  /* Part P: hail magnitude labels on storm-hit chips (same >=1.00" threshold as the PDF report) */
  #wlPop .wl-hailq{color:#7dd3a8;font-weight:600}
  #wlPop .wl-hailsub{color:#8b97a8;font-style:italic}`;
  ...
  d.innerHTML = "<div class='wl-bar' id='wlBar'><span class='wl-grip'>&#8942;&#8942;</span>"
    ...
    + "<div class='wl-vgrip' id='wlVGrip' title='Drag to lengthen'></div>";
  document.body.appendChild(d);
  document.getElementById("wlXBtn").onclick = wlClose;
  wlWireDrag(document.getElementById("wlBar"), d);
  wlWireVResize(document.getElementById("wlVGrip"), d);
}
// Part F: vertical-only resize from the bottom edge. Height is driven directly rather than through
// the CSS resize grip, so a drag can lengthen the panel without also widening it.
function wlWireVResize(grip, pop){
  if(!grip) return;
  let startY=0, startH=0, sizing=false;
  const MIN=240;
  grip.addEventListener("pointerdown", (e)=>{
    const r=pop.getBoundingClientRect();
    startY=e.clientY; startH=r.height; sizing=true;
    // Pin the top-left before resizing: the panel is positioned from `top/right` by default, so
    // growing the height while `right` is still authoritative makes it appear to jump sideways.
    pop.style.top=r.top+"px"; pop.style.left=r.left+"px"; pop.style.right="auto"; pop.style.bottom="auto";
    try{ grip.setPointerCapture(e.pointerId); }catch(_){}
    e.preventDefault(); e.stopPropagation();
  });
  grip.addEventListener("pointermove", (e)=>{
    if(!sizing) return;
    // Clamp to the viewport bottom so the panel cannot be dragged past where it can be seen.
    const top=pop.getBoundingClientRect().top;
    const max=Math.max(MIN, window.innerHeight - top - 8);
    pop.style.height=Math.max(MIN, Math.min(startH + (e.clientY-startY), max))+"px";
  });
...
// D4: render the (filtered, sorted) rows. Split out of wlLoad so a filter change re-renders without
// re-fetching -- at 200+ addresses one fetch and client-side work is the right trade, and the payload
// is already fully materialised by the API.
function wlRenderRows(){
  const b = document.getElementById("wlBody"); if(!b) return;
  const rows = wlApplyFilters(WL_ROWS);
  let h = wlFilterBar(rows.length, WL_ROWS.length) + "<div class='wl-list'>";
  if(!rows.length){
    h += "<div class='wl-msg'>No addresses match these filters. <button class='wl-back' id='wlClearF'>Clear filters</button></div></div>";
    b.innerHTML = h; wlWireFilterBar(); return;
  }
  h += "<table><thead><tr><th>Address</th><th>Name</th><th>Roof</th><th>Last hit</th><th>Filing window</th></tr></thead><tbody>";
  for(const r of rows){
    ...
    h += "<tr class='wl-row' data-pid='"+wlEsc(r.property_id)+"'>"
      ...
      +"<td>"+(r.last_hit_date?wlEsc(r.last_hit_date):"<span class='wl-na'>&mdash;</span>")+"</td>"
      +"<td>"+wlWindowChip(r)+"</td></tr>";
  }
  h += "</tbody></table></div>";
  b.innerHTML = h;
  wlWireFilterBar();
...
// D3: the claim-window chip. Tone comes from the API (storm.claim_window) -- the UI colours what it
// is told and never re-derives a class or a deadline, so this surface cannot drift from the address
// panel. ALWAYS says "estimated": the real deadline is set by the carrier and the policy.
function wlWindowChip(r){
  const tone = r.window_tone || "grey";
  const st   = r.window_status || "unknown";
  let txt;
  if(st === "open")         txt = (r.window_months_left!=null ? Math.round(r.window_months_left) : "?") + " mo left";
  else if(st === "closing") txt = "closes soon";
  else if(st === "closed")  txt = "window closed";
  else                      txt = r.use_class === "unknown" ? "classify" : "no qualifying hail";
  const cls = r.use_class && r.use_class !== "unknown"
    ? r.use_class.charAt(0).toUpperCase()+r.use_class.slice(1)
      + (r.window_months ? " \\u00b7 "+r.window_months+"mo" : "")
    : "unclassified";
  // title carries the full sentence + which cascade tier answered, so the operator can see whether a
  // classification came from the assessor or from an inference without opening the address.
  const tip = (r.window_label||"") + "  \\u2014  " + cls
            + (r.use_class_tier ? " (via "+r.use_class_tier+")" : "")
            + (r.use_class_basis ? ": "+r.use_class_basis : "")
            + "  \\u2014  estimated only; carrier and policy govern.";
  return "<span class='wl-cw wl-cw-"+wlEsc(tone)+"' title='"+wlEsc(tip)+"'>"+wlEsc(txt)+"</span>";
}
// ── D4: sort / search / filter ───────────────────────────────────────────────────────────────────
// Client-side over the already-fetched set. At the 200+ address design target that is one fetch and
...
function wlValueOf(r){
  ...
  // (125,391 parcels); zeroing them would silently drop an eighth of the market out of any range.
  return (r.parcel_value_known && r.parcel_market_value > 0) ? r.parcel_market_value : null;
}
function wlApplyFilters(rows){
  ...
  let out = (rows||[]).filter(r=>{
    ...
    if(f.peril && (r.last_hit_summary||"").toLowerCase().indexOf(f.peril) === -1) return false;
    if(f.win && (r.window_status||"unknown") !== f.win) return false;
    const v = wlValueOf(r);
    if(v === null){ if(!f.vunknown) return false; }
    else {
  ...
  const cmpDesc = (k) => (a,b) => String(b[k]||"").localeCompare(String(a[k]||""));
  if(f.sort === "hail")      out.sort(cmpDesc("last_hail_date"));
  else if(f.sort === "torn") out.sort(cmpDesc("last_tornado_date"));
  else if(f.sort === "value") out.sort((a,b)=>{
    // unknown-value rows sort LAST rather than as 0, so they are visibly parked, not silently
    // ranked at the bottom of a numeric scale they were never on.
    const av=wlValueOf(a), bv=wlValueOf(b);
    if(av===null && bv===null) return String(a.address||"").localeCompare(String(b.address||""));
    if(av===null) return 1;
    if(bv===null) return -1;
    return bv-av;
  });
  else if(f.sort === "window") out.sort((a,b)=>{
    const rank = {closing:0, open:1, closed:2, unknown:3};
    const d = (rank[a.window_status]??9) - (rank[b.window_status]??9);
    return d || ((a.window_months_left??999) - (b.window_months_left??999));
  });
  else out.sort((a,b)=> String(a.address||"").localeCompare(String(b.address||"")));
  return out;
}
function wlFilterBar(shown, total){
  const f = WL_F;
  ...
  return "<div class='wl-filters'>"
    ...
    +     opt("addr","Sort: address",f.sort)+opt("hail","Sort: last hail",f.sort)
    +     opt("torn","Sort: last tornado",f.sort)+opt("value","Sort: property value",f.sort)
    +     opt("window","Sort: filing window",f.sort)
    +   "</select>"
    + "</div>"
    + "<div class='wl-frow'>"
    +   "<select id='wlPeril' class='wl-in'>"+opt("","Any peril",f.peril)+opt("hail","Hail",f.peril)
    +     opt("wind","Wind",f.peril)+opt("tornado","Tornado",f.peril)+"</select>"
    +   "<select id='wlWin' class='wl-in'>"+opt("","Any window",f.win)
    +     opt("open","Window open",f.win)+opt("closing","Closing soon",f.win)
    +     opt("closed","Window closed",f.win)+opt("unknown","Window unknown",f.win)+"</select>"
    + "</div>"
    + "<div class='wl-frow'>"
    +   "<input id='wlVMin' class='wl-in wl-num' type='number' placeholder='Value min' value='"+(f.vmin??"")+"'>"
    +   "<input id='wlVMax' class='wl-in wl-num' type='number' placeholder='Value max' value='"+(f.vmax??"")+"'>"
    +   "<label class='wl-chk' title='Jefferson and Monroe counties carry no assessor value at all "
    +     "(125,391 parcels). Untick to exclude them; they are never treated as $0.'>"
    +     "<input id='wlVUnk' type='checkbox'"+(f.vunknown?" checked":"")+"> value unknown</label>"
    + "</div>"
...
function wlWireFilterBar(){
  ...
  const q=document.getElementById("wlQ");
  if(q){ q.addEventListener("input", ()=>{ WL_F.q=q.value; const p=q.selectionStart; wlRenderRows();
          const n=document.getElementById("wlQ"); if(n){ n.focus(); try{n.setSelectionRange(p,p);}catch(_){}} }); }
  on("wlSort","change",e=>{ WL_F.sort=e.target.value; wlRenderRows(); });
  on("wlPeril","change",e=>{ WL_F.peril=e.target.value; wlRenderRows(); });
  on("wlWin","change",e=>{ WL_F.win=e.target.value; wlRenderRows(); });
...
function wlHeightAccess(r){
  ...
  for(const sec of order){
    ...
    if(v.eave_ft==null){   // per-side refused - a tree over the rear does not blank the front
      const why = v.status==="no_ground" ? "no ground data" : "occluded";
      rows += "<tr><td class='wl-hgt-sec'>"+sec+"</td><td colspan='4' class='wl-hgt-noread'>no reliable read &mdash; "+why+", verify on site</td></tr>";
      continue;
    }
    const f=flat[sec];
...
async function wlDetail(pid){
  ...
  // (watchlist.storm_hits_on_record -> report.hail_history: same source as the PDF + claim_window),
  // so a sub-1.00" hail day reads as "below threshold" on screen and can't look like a qualifying hit.
  for(const x of hits){
    let mag = "";
    if(x.hail_size_in != null){
      const s = Number(x.hail_size_in).toFixed(2);
      mag = x.hail_reportable
        ? " &middot; <span class='wl-hailq' title='Qualifying hail (\\u2265 1.00\\u2033 at the address)'>"+s+"\\u2033</span>"
        : " &middot; <span class='wl-hailsub' title='Below the 1.00\\u2033 qualifying threshold'>"+s+"\\u2033 below threshold</span>";
    }
    h += "<span class='wl-chip'><span class='wl-date' data-d='"+wlEsc(x.date)+"'>"+wlEsc(x.date)+"</span> &middot; "+wlEsc((x.perils||[]).join(", "))+mag+"</span>";
  }
  h += "</div></div>";
  // Parcel (e)
  ...
  function collectFields(){
    const out={};
    for(const f of WL_STR_FIELDS.concat(WL_NUM_FIELDS)){ const el=document.getElementById("wlf_"+f[0]); if(el) out[f[0]]=el.value; }
    const ar=document.getElementById("wlf_alert_recipients"); if(ar) out.alert_recipients=ar.value;
    return out;
  }
  ...
  document.getElementById("wlRemove").onclick = async ()=>{
    ...
    try{ const rr=await sdApi("watchlist-remove",{property_id:pid});
      // Part L symmetry: on remove, the un-watched address drops off BOTH surfaces immediately.
      // Drop this pin synchronously FIRST (no network wait), then rebuild the list and reconcile the
      // pin layer. The synchronous drop is what guarantees "the pin goes away" even before the
      // re-fetch returns.
...
// Part G: the Google-style teardrop used for watched addresses. Built as an inline-SVG divIcon so it
// carries no external asset (the portal is a single self-contained page) and can take the row's own
// marker_color. The white stroke keeps it legible over dark hail fill and light basemap alike.
function wlPinIcon(col){
  const svg =
    "<svg xmlns='http://www.w3.org/2000/svg' width='26' height='34' viewBox='0 0 26 34'>"
    + "<path d='M13 33C13 33 24.5 19.8 24.5 12.4 24.5 5.6 19.4.5 13 .5S1.5 5.6 1.5 12.4C1.5 19.8 13 33 13 33Z' "
    +   "fill='" + col + "' stroke='#fff' stroke-width='2' stroke-linejoin='round'/>"
    + "<circle cx='13' cy='12.2' r='4.2' fill='#fff' fill-opacity='.92'/></svg>";
  return L.divIcon({
    html: svg, className: "wl-pin-icon",
...
// `exclude` drops those property_ids even if the fetched list still contains them -- used right after
// a remove so a momentarily-stale read can never resurrect the pin the operator just deleted.
async function wlBuildPinMarkers(exclude){
  const skip = exclude instanceof Set ? exclude : new Set(exclude||[]);
  const res = await sdApi("watchlist-list", {});
  const rows = ((res && res.watched) || []).filter(r=> !skip.has(r.property_id));
  const markers = [];
  for(const r of rows){
    ...
    m._wlPid=pidv;                     // tag so a single pin can be dropped without a re-fetch
...
// answers {status:"warming"} INSTANTLY for a cold date (warming the core in the background) and the
// result once it lands; sgApi POLLS this same endpoint until it stops warming, so no request ever
// runs long enough to time out. A warm date returns the result on the first call (no polling).
async function sgApiOnce(body){
...
async function sgApi(body, onWarm){
  const DEADLINE = Date.now() + 300000;   // 5-min ceiling: a cold solve is ~200s; never hang forever
  let waited = false;
  while(true){
    const j = await sgApiOnce(body);
    if(j && j.status === "warming"){
      waited = true;
      if(typeof onWarm === "function") onWarm();
      if(Date.now() > DEADLINE) throw new Error("solve timed out while warming (>5 min) — try again");
      await new Promise(res=>setTimeout(res, 4000));       // poll every 4s while the core warms
      continue;
    }
    if(j && j.status === "error") throw new Error(j.error || "solve error");
    if(waited && j && typeof j === "object") j.warmed_from_cold = true;   // note for the caller/UI
    return j;
  }
}
...
async function sgSolveFresh(){   // hail floor / date -> FRESH geometry from the engine (the only re-solve)
  ...
  const t0 = Date.now();
  // while a cold date warms, keep the operator informed with an elapsed counter instead of a frozen
  // "Solving…" (the poll can run a couple of minutes on a never-before-solved date).
  const onWarm = () => { const s = Math.round((Date.now()-t0)/1000);
    if(w) w.innerHTML="Warming this date\\u2026 <span class='warm'>first solve builds the parcel cores (~2-3 min) \\u00b7 "+s+"s</span>";
    if(v) v.textContent="Warming this date\\u2026 "+s+"s"; };
  try {
    const sol = await sgApi({ date:getDate(), peril:SG_PERIL,
      hail_floor:+document.getElementById("sdHail").value,   // ignored server-side for tornado (fixed corridor)
      spend_cap:+document.getElementById("sdCapR").value }, onWarm);
...
function sgRenderAll(){
  ...
  if(v){
    // Core-3: a tornado solve carries a peril tag + a PARCEL-BASIS label so its $ is never mistaken for
    // the hail damaged-homes basis. Hail solves (no exposure_basis) render byte-identically to before.
    const isT = (sol.peril && sol.peril!=="hail");
    const perilTag = isT ? "<span class='sd-peril'>"+sol.peril.toUpperCase()+"</span> " : "";
    const basis = sol.exposure_basis ? " <span class='sd-basis' title='"+sol.exposure_basis+"'>PARCEL-BASIS</span>" : "";
    v.innerHTML = perilTag + (s.dmg>0 ? ("Solved \\u00b7 "+s.n+" rings \\u00b7 $"+Math.round(s.smin).toLocaleString()+"\\u2013$"+Math.round(s.smax).toLocaleString()+basis)
      : "Presence only \\u2014 no damage core; recommended $0");
    v.className="sd-verdict "+(s.dmg>0?"ok":"")+(sol.exposure_basis?" parcelbasis":""); }
...
function sgHeadline(sol){
  ...
  w.innerHTML="floor <b>"+(+sol.floor).toFixed(3)+"\\u2033</b> \\u00b7 <b>"+s.n+"</b> rings \\u00b7 audience <b>"+s.aud.toLocaleString()
    ...
    +((sol.served_by==="cold"||sol.index_warming)?' <span class="warm" title="The resident solve engine (KCCSolveEngine, 8790) did not serve this solve \\u2014 the slow cold GPKG path did. Numbers are correct; expect delay. If every solve is cold, the service is down or stale (restart it).">\\u00b7 \\u26a0 cold solve \\u2014 resident engine unavailable or stale; expect delay / check service</span>':"");
...
// circle_id -> coords for the Solve map SNAP, keyed by the ACTIVE peril's cluster:
// CLUSTER-<peril>-<date>#<idx> (matches solve()'s funded_circle_ids / deselected[].circle_id).
// Rebuilt on every peril flip so the snap lands on the right geometry.
function sdBuildCircles(peril, D){
  SD_CIRCLES = {};
  const src = (peril === "tornado") ? ((D && D.tornado && D.tornado.circles) || [])
                                    : ((D && D.circles) || []);
  const ck = "CLUSTER-" + peril + "-" + getDate();
  src.forEach((c, i) => { SD_CIRCLES[ck + "#" + i] = { lat: c.center_lat, lng: c.center_lng, r: c.radius_mi }; });
}
// Tornado button state off the per-date tornado gate verdict the proxy serves (D.chips, from
// storm_date_layers' fundable / not_promotable_reason). Fundable -> enabled; detected-but-not-
// fundable -> disabled with the states-why reason (never "calibrating"); no tornado row -> disabled
// "no tornado detected". Wind stays "calibrating" (no wind solve exists).
function sdTornadoBtn(D){
  const chips = (D && D.chips) || {}, f = chips.tornado_fundable;
  if(f === true)  return '<button data-peril="tornado">Tornado</button>';
  const why = (f === false) ? (chips.tornado_reason || "detected \\u2014 below the tornado funding gate")
                            : "no tornado detected on this date";
  return '<button disabled data-peril="tornado" title="' + why.replace(/"/g,"&quot;") + '">Tornado</button>';
}
// Flip the active Spend-Dial peril: restyle the segment, swap the header badge, disable the hail-floor
// slider for tornado (a tornado prices on its FIXED corridor — the floor is meaningless, never let it
// silently pretend to re-solve), rebuild the snap circles, and re-price the table for the new peril.
function sdSetPeril(peril, D){
  if(peril === SG_PERIL) return;
  SG_PERIL = peril;
  document.querySelectorAll("#spendPanel .sd-peril button").forEach(b =>
    b.classList.toggle("on", b.dataset.peril === peril));
  const pk = document.querySelector("#spendPanel h2 .pk"); if(pk) pk.textContent = peril.toUpperCase();
  const hail = document.getElementById("sdHail"), hrow = hail ? hail.closest(".sd-row") : null;
  const isT = (peril === "tornado");
  if(hail) hail.disabled = isT;
  if(hrow){ hrow.style.opacity = isT ? "0.45" : ""; hrow.title = isT ? "Hail floor is n/a for a tornado solve \\u2014 it prices on the fixed corridor" : ""; }
  const fn = document.querySelector("#spendPanel .sd-floornote");
  if(fn) fn.textContent = isT
    ? "n/a for tornado \\u2014 the corridor geometry is fixed; funding is corridor parcels (audience), priced parcel-basis."
    : "Audience width only \\u2014 funding comes from the damage cores (\\u22658.5 intensity or \\u22652.0\\u2033 size). Dialing below 1.80\\u2033 widens the ad field around funded damage; it never changes damaged homes or spend.";
...
function setupSpendDial(D){
  ...
  p.innerHTML =
    ...
    + '<div class="sd-row"><label title="Audience width \\u2014 which cells qualify a ring to run ads. Not the money.">Hail floor</label><input id="sdHail" type="range" min="1.125" max="4.00" step="0.025" value="1.25"><span class="val" id="sdHailV">1.25\\u2033</span></div>'
    ...
    + '<button id="sdReshape" title="Reshape to win cheap (lever E) \\u2014 recenter/re-radius/bid the running campaign\\u2019s circles onto the hardest-hit, cheapest-to-win inventory from live win-rates. Shows before (grey) vs proposed (green dashed) on the map; you confirm before anything changes.">Reshape to win cheap</button>'
  ...
  { const _rb=document.getElementById("sdReshape"); if(_rb) _rb.onclick = sgReshapeWinCheap; }  // lever E
...
function sdPopup(cid){
  ...
  pop.innerHTML = "<h3>Ring #" + cid.split("#")[1] + "<button class='sd-pop-x'>&#10005;</button></h3>"
    ...
    + (function(){ var d1=r.core1_damaged||0, d2=r.core2_damaged||0, dm=r.damaged_homes||0;
        var tag = dm<=0 ? "<span class='cprov presence'>presence only</span>"
          : (d1>0&&d2>0 ? "<span class='cprov both'>C1+C2</span>" : (d2>0 ? "<span class='cprov c2'>C2 \\u22652.0\\u0022 size</span>" : "<span class='cprov c1'>C1 \\u22658.5 intensity</span>"));
        return "<div class='r'><span>damage core</span><b>" + tag + "</b></div>"; })()
...
function sgPromoDials(){ return { date:getDate(), peril:SG_PERIL,
...
async function sgRefreshPromotion(){
  ...
  if(info && info.promoted){
    ...
    pr.innerHTML="<span class='badge'>PROMOTED \\u00b7 v"+info.promotion_version+" \\u00b7 "+(info.promoted_at||"")+"</span>"
      ...
      +(info.finalized_differs ? "<span class='mc'>\\u26a0 FINALIZED GEOMETRY DIFFERS FROM PROMOTED \\u2014 review before continuing spend</span>" : "")
      +(info.campaign_record_exists ? "<span class='cx'>\\u26a0 campaign exists \\u2014 re-promotion changes NOTHING at Google; mid-flight re-pointing is Stage 3</span>" : "")+_pi;
  } else {
    send.disabled=true;
...
async function sgGoLiveLaunch(){
  ...
  const close=function(){ if(typeof sgClearReshapeLayer==="function") sgClearReshapeLayer(); if(bk.parentNode) bk.parentNode.removeChild(bk); };
...
async function sgRefreshLive(){
  if(typeof sgClearReshapeLayer==="function") sgClearReshapeLayer();  // storm/date/peril switch (+refresh): never carry a prior storm's reshape overlay onto this one
...
// LEVER E (2026-09-12): reshape / fit-to-win-cheap. Preview reads live win-rates + proposes recenter /
// re-radius / bid moves; the map shows CURRENT (grey) vs PROPOSED (green dashed); the operator confirms and
// the gated apply pushes. SG_RESHAPE_LAYER is cleared on confirm / cancel / failure.
let SG_RESHAPE_LAYER = null;
function sgClearReshapeLayer(){
  try{ if(SG_RESHAPE_LAYER && TMAP && TMAP.hasLayer(SG_RESHAPE_LAYER)) TMAP.removeLayer(SG_RESHAPE_LAYER); }catch(e){}
  SG_RESHAPE_LAYER = null;
}
function sgDrawReshape(moves){
  sgClearReshapeLayer();
  SG_RESHAPE_LAYER = L.layerGroup();
  let drawn=0, bad=0;
  moves.forEach(function(m){
    try{
      const bc = (m.before && m.before.center) ? m.before.center : m.new_center;   // tighten/grow: center == new_center
      const br = (m.before && m.before.radius_mi) ? m.before.radius_mi : (m.new_radius_mi||1.6);
      const ac = m.new_center || (m.before && m.before.center) || bc;
      const ar = m.new_radius_mi || br;
      if(bc && bc.length===2) L.circle([bc[0], bc[1]], {radius: br*1609.34, color:"#94a3b8", weight:1, fill:false}).addTo(SG_RESHAPE_LAYER);
      if(ac && ac.length===2 && (m.new_center || m.new_radius_mi)) L.circle([ac[0], ac[1]], {radius: ar*1609.34, color:"#4ade80", weight:2, dashArray:"5,4", fill:false}).addTo(SG_RESHAPE_LAYER);
      drawn++;
    }catch(e){ bad++; console.warn("[reshape] skipped a bad move ("+(m&&m.circle_id)+")", e); }  // one bad move must not blank the whole before/after view
  });
  if(bad) console.warn("[reshape] drew "+drawn+" of "+moves.length+" moves ("+bad+" skipped)");
  try{ SG_RESHAPE_LAYER.addTo(TMAP); }catch(e){ console.error("[reshape] overlay add failed", e); }
}
function sgReshapeLabel(m){
  if(m.action==="bid_bump") return "raise bid to $" + (((m.new_cpc_micros||0)/1e6).toFixed(2));
  if(m.action==="recenter") return "recenter onto hardest-hit";
  if(m.action==="recenter_tighten") return "recenter + tighten to " + ((m.new_radius_mi||0).toFixed(2)) + "mi";
  if(m.action==="tighten") return "tighten to " + ((m.new_radius_mi||0).toFixed(2)) + "mi";
  if(m.action==="grow") return "grow to " + ((m.new_radius_mi||0).toFixed(2)) + "mi";
  return m.action;
}
async function sgReshapeWinCheap(){
  const v=document.getElementById("sdVerdict");
  if(!SG_LIVE){ if(v){ v.textContent="Reshape needs a LIVE campaign \\u2014 launch / Go Live first."; v.className="sd-verdict warn"; } return; }
  if(v){ v.textContent="Reading live win-rates \\u2014 finding cheaper-to-win geometry\\u2026"; v.className="sd-verdict"; }
  let res;
  try{ res=await sdApi("lever-e-preview",{date:getDate(), peril:SG_PERIL||"hail"}); }
  catch(e){ sgClearReshapeLayer(); if(v){ v.textContent="Reshape preview failed \\u2014 "+e.message; v.className="sd-verdict warn"; } return; }
  const moves=((res&&res.proposals)||[]).filter(function(p){ return p.action && p.action!=="hold"; });
  if(!moves.length){
    sgClearReshapeLayer();
    if(v){ v.textContent="Nothing to reshape \\u2014 every funded ring is holding (winning cheaply, or not enough data yet)."; v.className="sd-verdict ok"; }
    return;
  }
  sgDrawReshape(moves);
  const rows=[["Storm", getDate()+" \\u00b7 "+(SG_PERIL||"hail")]]
    .concat(moves.slice(0,12).map(function(m){ return [String(m.circle_id||"ring"), sgReshapeLabel(m)]; }));
  if(moves.length>12) rows.push(["\\u2026", "+"+(moves.length-12)+" more"]);
  sgLiveConfirm({
    title:"\\u26a0 Reshape to win cheap \\u2014 push "+moves.length+" change(s)?",
    cls:"go", confirm:"Yes, reshape live", working:"Reshaping\\u2026",
    rows:rows,
    warn:"Grey = current, green dashed = proposed (on the map). Each change recenters / re-radiuses / re-bids "+
         "the <b>running</b> campaign toward the hardest-hit, cheapest-to-win inventory from live win-rates. "+
         "It mutates live targeting / bids in place (never recreates, keeps serving). The engine re-validates "+
         "every move against the current bounds and REFUSES any out of range. Confirm only if reviewed.",
    onGo:async function(done){
      let ar;
      try{ ar=await sdApi("lever-e-apply",{date:getDate(), peril:SG_PERIL||"hail", approved:moves, i_reviewed:true, validate_only:false}); }
      catch(e){ done(); sgClearReshapeLayer(); if(v){ v.textContent="Reshape failed \\u2014 "+e.message; v.className="sd-verdict warn"; } sgRefreshLive(); return; }
      done(); sgClearReshapeLayer();
      if(!ar || ar.ok===false){
        const why=(ar&&(ar.states_why||(ar.push&&ar.push.states_why)))||"the server refused this action";
        if(v){ v.textContent="Reshape refused \\u2014 "+why; v.className="sd-verdict warn"; } }
      else {
        const n=(ar.overrides_written&&ar.overrides_written.length)||0;
        const ref=(ar.refused&&ar.refused.length)||0;
        if(v){ v.textContent="Live campaign reshaped \\u2014 "+n+" ring(s) pushed"+(ref?(", "+ref+" refused (out of bounds)"):"")+" for "+getDate()+"."; v.className="sd-verdict ok"; } }
      sgRefreshLive();
    }});
}
...
function setupMobile(){
  ...
  // CAMPAIGN MONITOR (2026-08-22): its own full-width overlay (not a bottom sheet), so it gets a
  // direct launcher button rather than a data-sheet entry. Rule 18: renders on phone too.
  const cbtn=document.createElement("button"); cbtn.id="mCampaigns"; cbtn.textContent="\\ud83d\\udce3";
  cbtn.title="Campaigns (live portfolio)"; cbtn.onclick=()=>{ if(window.tempestOpenCampaigns) window.tempestOpenCampaigns(); }; tb.appendChild(cbtn);
...
function buildChaseLayers(chase){
  ...
  // OUT-OF-MARKET confirmed hail dots (chase/pull, 2026-08-23) — FILTERED to the ad-funding diameter
  // (1.25" = AD_ELIGIBILITY_HAIL_FLOOR) so the chase view shows the SAME size roofs you'd fund in-market.
  // bindPopup = mobile-safe (persistent tap; a bare tooltip flashes on touch). Keep in sync w/ the Hail-floor dial.
  const CHASE_EV_FLOOR = 1.25;
  (chase.evidence||[]).filter(p=>(+p.size_in||0) >= CHASE_EV_FLOOR).forEach(p=>{
    const _sz=(p.size_in!=null?p.size_in:"?");
    L.circleMarker([p.lat,p.lng],{pane:"chaseCircPane",radius:3+Math.min(+p.size_in||1,3)*1.3,
      ...
      .bindTooltip((p.source||"")+" "+_sz+'"'+(p.city?" \\u00b7 "+p.city:""))
      .bindPopup("<b>"+(p.source||"report")+"</b> reported <b>"+_sz+'"</b>'+(p.city?"<br>"+p.city:"")+"<br><i>out-of-market \\u00b7 chase/pull candidate</i>").addTo(evidence);
  });
...
let CHASE_SWATH_LAYER=null, CHASE_LBL=null, CHASE_LBL_UPD=null;
...
function addChaseSwathLayer(D){
  if(CHASE_LBL_UPD && TMAP){ TMAP.off("moveend zoomend", CHASE_LBL_UPD); CHASE_LBL_UPD=null; }
  CHASE_LBL=null; CHASE_HULL_PTS=null;
  ...
  // LEGIBILITY (2026-07-20): this hull used to be #7c869a dash 7,6 — a grey-blue dashed line
  // almost identical to the market geofence (#90a4ae dash 2,3). The operator read the hull AS
  // the market boundary and concluded "STL is inside the box but there is no swath = broken".
  // It is now AMBER with a long dash-dot: a warm colour the geofence never uses, and a pattern
  // nothing else on the map draws. Plus a label ON the shape saying what it is NOT.
  if(hull.length>=3){
    layers.push(L.polygon(hull,{pane:"chaseSwathPane",color:"#c98a2e",weight:2.2,
      dashArray:"14,5,3,5",fill:false,interactive:false,opacity:.95}));
    // Anchor the label at the hull vertex NEAREST THE MAP CENTRE, and re-anchor on pan/zoom.
    //
    // It first shipped anchored to the hull's NORTHERNMOST vertex. On 07-18 that put the label at
    // y = -453px — off the top of the viewport — so the operator's review screenshot showed the
    // amber hull with no label at all, which is the one thing the label existed to prevent. A
    // fixed vertex cannot be "legible at default zoom" for every date, because the hull's shape
    // and the view are independent. Tracking the centre keeps it on the shape AND in front of
    // whoever is looking.
    CHASE_HULL_PTS=hull;
    CHASE_LBL=L.marker(hull[0],{pane:"chaseSwathPane",interactive:false,
      icon:L.divIcon({className:"chasehull-lbl",html:"chase activity extent \\u2014 not the market boundary",
        iconSize:[0,0],iconAnchor:[0,10]})});
    layers.push(CHASE_LBL);
    CHASE_LBL_UPD=()=>{
      if(!CHASE_LBL || !TMAP) return;
      const c=TMAP.getCenter(), b=TMAP.getBounds();
      // Walk the hull BOUNDARY, not just its vertices, and prefer a point that is actually on
      // screen. Nearest-vertex was not enough: on 07-18 the hull's EDGE crosses the viewport
      // while every vertex lies outside it, so the operator saw the amber line with no label.
      // Sampling each edge finds the on-screen stretch of boundary and pins the label there.
      // Step sized to the VIEW, not a fixed sample count: a fixed count can stride clean over a
      // zoomed-in viewport when the hull edge is long. Capped per edge so a wide zoom stays cheap
      // (measured 0 ms on a CONUS-scale view).
      const span={lat:Math.abs(b.getNorth()-b.getSouth()), lng:Math.abs(b.getEast()-b.getWest())};
      const step=Math.max(1e-4, Math.min(span.lat, span.lng)/3);
      let onScreen=null, nearest=hull[0], nd=Infinity;
      for(let i=0;i<hull.length && !onScreen;i++){
        const a=hull[i], z=hull[(i+1)%hull.length];
        const elen=Math.max(Math.abs(z[0]-a[0]), Math.abs(z[1]-a[1]));
        const SEG=Math.min(600, Math.max(1, Math.ceil(elen/step)));
        for(let t=0;t<=SEG;t++){
          const p=[a[0]+(z[0]-a[0])*t/SEG, a[1]+(z[1]-a[1])*t/SEG];
          if(b.contains(p)){ onScreen=p; break; }
          const dy=p[0]-c.lat, dx=(p[1]-c.lng)*Math.cos(c.lat*Math.PI/180);
          const d=dy*dy+dx*dx; if(d<nd){ nd=d; nearest=p; }
        }
      }
      // Nothing on screen => the whole chase hull is off-view; park it at the closest boundary
      // point so the label arrives the moment you pan toward the swath.
      CHASE_LBL.setLatLng(onScreen||nearest);
    };
    CHASE_LBL_UPD();
    TMAP.on("moveend zoomend", CHASE_LBL_UPD);
  }
  ...
  if(host){ const b=document.createElement("div"); b.className="chaseoom";
    ...
    b.title='CHASE — real hail, but OUTSIDE the KCC market geofence. Ring 2 chase territory: '
      +'never funded, never counted as in-market damage.\\n'
      +'gated = radar cells at or above the 1.125\\u2033 funding gate.\\n'
      +'sq mi = square miles of gated footprint.\\n'
      +'The amber dash-dot outline is the extent of this chase activity — it is NOT the market boundary.';
    host.appendChild(b); }
}
...
function addSwathVisibilityFloor(D){
  ...
  SWATH_FLOOR_MARKER=L.circleMarker([lat,lng],{pane:"swathFloorPane",radius:9,color:"#fff",weight:2,
    ...
    // LEGIBILITY (2026-07-20): the WHY-line. A bare dot with numbers left the operator asking
    // why there was a marker instead of a swath. The dot is a PRESENTATION floor — the swath is
    // real and drawn, it is just a few pixels at this zoom — so say that, and say it resolves
    // by zooming in, which is the actual next action.
    .bindTooltip((sub?'Sub-gate hail':'Hail')+' \\u00b7 max '+n.max.toFixed(2)+'\\u2033 \\u00b7 '+n.gated+
      ' gated cell'+(n.gated===1?'':'s')+' \\u00b7 '+n.sqmi+' sq mi'+
      '<br><span class="whyline">cluster too small to draw at this zoom ('+n.gated+' cell'+
      (n.gated===1?'':'s')+' \\u00b7 '+n.sqmi+' sq mi) \\u2014 shown as marker; zoom in for the real footprint</span>',
      {direction:"top"});
  const FLOOR_ZOOM=11;
...
// ── ON-MAP TIER LEGEND (2026-07-20 legibility pass) ──────────────────────────────────────────
//    The tier vocabulary (fundable / sub-gate / chase) existed ONLY in the calendar popup, so the
//    map never said which tier was on screen — the operator had to infer it from colour. This is
//    the SAME vocabulary and the SAME fills, so one legend teaches both surfaces.
//    It also names the two dashed boundaries that were being confused for each other, which is
//    the whole reason this pass exists. Collapsed state persists in localStorage.
//    DISPLAY ONLY: reads nothing, decides nothing, renders no data — a static key. ──
let TIER_LEGEND_CTL=null;
function addTierLegend(){
  if(!TMAP) return;
  if(TIER_LEGEND_CTL){ try{ TMAP.removeControl(TIER_LEGEND_CTL); }catch(e){} TIER_LEGEND_CTL=null; }
  const ex=document.getElementById("tierLegend"); if(ex) ex.remove();
  const el=document.createElement("div"); el.id="tierLegend";
  const closed=localStorage.getItem("tierLegendClosed")==="1";
  if(closed) el.classList.add("closed");
  el.innerHTML=
    '<div class="tl-h">Map key<span class="tl-caret">\\u25be</span></div>'+
    '<div class="tl-b">'+
      '<div class="tl-sec">Hail tier</div>'+
      '<div class="tl-row"><span class="tl-sw" style="background:#d33f2a"></span>fundable (at/above gate)</div>'+
      '<div class="tl-row"><span class="tl-sw" style="background:#d33f2a;opacity:.32"></span>sub-gate (below gate)</div>'+
      '<div class="tl-row"><span class="tl-sw" style="background:transparent;border:1.5px solid #8a94a8"></span>chase (out of market)</div>'+
      '<div class="tl-sec">Other perils</div>'+
      '<div class="tl-row"><span class="tl-sw" style="background:#7a2a20"></span>tornado</div>'+
      '<div class="tl-row"><span class="tl-sw" style="background:#24487f"></span>wind</div>'+
      '<div class="tl-sec">Boundaries</div>'+
      '<div class="tl-row"><span class="tl-ln" style="border-top:2px dashed #c98a2e"></span>chase activity extent</div>'+
      '<div class="tl-row"><span class="tl-ln" style="border-top:1.5px dotted #5b7fa6"></span>KCC market geofence</div>'+
    '</div>';
  const h=el.querySelector(".tl-h");
  h.addEventListener("click", ()=>{ el.classList.toggle("closed");
    localStorage.setItem("tierLegendClosed", el.classList.contains("closed")?"1":"0"); });
  if(window.L && L.DomEvent){ L.DomEvent.disableClickPropagation(el); L.DomEvent.disableScrollPropagation(el); }
  // Leaflet owns the placement: the top-left stack already holds the zoom control and the hover
  // readout, and Leaflet flows a new control BELOW them. Added last => sits last, no overlap.
  TIER_LEGEND_CTL=L.control({position:"topleft"});
  TIER_LEGEND_CTL.onAdd=()=>el;
  TIER_LEGEND_CTL.addTo(TMAP);
}
...
function setupFloatPanels(){
  ...
  defs.forEach(function(def){
    ...
    bar.appendChild(btn);
    if(!sel){ btn.onclick=function(){                                   // selector-less tools (own overlay)
        if(id==="campaigns"){ if(window.tempestOpenCampaigns) window.tempestOpenCampaigns(); }
        else if(window.tempestOpenWx) window.tempestOpenWx(); }; return; }   // Weather tool
...
// ===================== CAMPAIGN MONITOR (Design Lock v3 §13 portfolio / §12 banner) =====================
// READ-ONLY operator surface over the live Google Ads account (punch-list #2 panel + #5 banner view).
// Fires sdApi("campaign-status") -> the netlify read verb -> KCCStormAPI /campaign_status -> the engine
// (a live GAQL SELECT + pacing/stage/flag compute; NO account or graph writes). A right-side overlay
// (full-width on phones), stage-sorted, quiets aging campaigns (§13). Rule-1: every conversion-derived
// figure is badged MODELED/proxy (conversions aren't flowing yet). Rule 11: account/window/as-of show in
// the context strip without scrolling. Rule 22: refresh preserves the body scroll position (no jump).
function initCampaigns(){
  if(document.getElementById("cmpPanel")) return;
  const ov=document.createElement("div"); ov.id="cmpPanel"; ov.className="cmp-panel hidden";
  ov.innerHTML=
    '<div class="cmp-hd"><b>\\ud83d\\udce3 Campaigns</b><span class="cmp-sub">live portfolio \\u00b7 read-only</span><button class="cmp-x" title="Close">\\u00d7</button></div>'+
    '<div class="cmp-ctx"></div>'+
    '<div class="cmp-modeled"></div>'+
    '<div class="cmp-body"><div class="cmp-load">Loading campaigns\\u2026</div></div>';
  document.body.appendChild(ov);
  const bk=document.createElement("div"); bk.id="cmpBackdrop";
  bk.style.cssText="position:fixed;inset:0;z-index:2400;background:rgba(0,0,0,.35);display:none";
  document.body.appendChild(bk);

  function cmpOpen(){ ov.classList.remove("hidden"); bk.style.display="block"; cmpLoad(); }
  function cmpClose(){ ov.classList.add("hidden"); bk.style.display="none"; }
  ov.querySelector(".cmp-x").onclick=cmpClose;
  bk.onclick=cmpClose;

  const money=n=>(n==null)?"\\u2014":"$"+Number(n).toLocaleString(undefined,{maximumFractionDigits:2});
  const pct=f=>(f==null)?"\\u2014":Math.round(Number(f)*100)+"%";
  const num=n=>Number(n||0).toLocaleString();
  const esc=s=>String(s==null?"":s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
  const stgCls=s=>String(s||"").replace(/[^A-Za-z]/g,"");   // "wound-down" -> "wounddown" (valid class)

  function renderCtx(rep){
    const as=rep.as_of? String(rep.as_of).replace("T"," ").slice(0,16):"\\u2014";
    const win=rep.window? String(rep.window).replace("segments.date ",""):"\\u2014";
    ov.querySelector(".cmp-ctx").innerHTML=
      '<span>Account <b>'+esc(rep.account||"\\u2014")+'</b></span>'+
      '<span>Window <b>'+esc(win)+'</b></span>'+
      '<span>As of <b>'+esc(as)+'</b></span>';
  ...
  function renderDetail(rep){
    ...
    let flashHtml='';
    if(glFlash && glFlash.campaign===c.campaign){
      flashHtml='<div class="cmp-gl-msg '+(glFlash.err?"err":"ok")+'">'+esc(glFlash.text)+'</div>'; glFlash=null; }
    h+='<div class="cmp-golive">'+
       '<div class="cmp-gl-hd"><span class="cmp-sec-t" style="margin:0">Campaign control</span>'+
       '<span class="cmp-gl-state '+(c.serving?"live":"paused")+'">'+(c.serving?"LIVE":"PAUSED")+'</span></div>'+
       '<div class="cmp-gl-sub">Storm <b>'+esc(c.date)+'</b> \\u00b7 '+esc(c.peril||"hail")+
       ' \\u00b7 daily budget <b>'+money(c.daily_budget)+'</b></div>'+
       (c.serving
         ? '<button class="cmp-gl-btn pause" data-gl="pause">\\u23f8 Pause campaign (stop serving)</button>'
         : '<button class="cmp-gl-btn go" data-gl="go">\\u25b6 Go Live \\u2014 start spending real money</button>')+
       flashHtml+
       '</div>';
    const bn=c.banner||{};
    ...
    if(!st || !(st.terms && st.terms.length)){ h+='<div class="cmp-empty">No search-term rows in the window.</div>'; }
    else {
      h+='<div class="cmp-stmeta">'+
         '<span class="cmp-basis '+(st.proxy?"proxy":"measured")+'">'+(st.proxy?"proxy basis":"measured")+'</span>'+
         '<span>Wasted spend <b>'+money(st.total_wasted_spend)+'</b></span>'+
         '<span>'+num(st.n_terms)+' terms</span></div>';
      if(st.proxy && st.proxy_note){ h+='<div class="cmp-stnote">'+esc(st.proxy_note)+'</div>'; }
      const is=st.intent_summary||{}; const ks=Object.keys(is);
      if(ks.length){ h+='<div class="cmp-stclasses">'; ks.forEach(function(k){
        h+='<span class="cmp-cls">'+esc(k.replace(/_/g," "))+' <b>'+num(is[k].count)+'</b> \\u00b7 '+money(is[k].spend)+'</span>'; });
        h+='</div>'; }
      h+='<div class="cmp-sts">';
      st.terms.forEach(function(t){
        let fl='';
        if(t.already_excluded){ fl+='<span class="cmp-stflag excl">already negated</span>'; }
        else if(t.already_added){ fl+='<span class="cmp-stflag added">keyword</span>'; }
        if(t.junk){ fl+='<span class="cmp-stflag junk">junk intent</span>'; }
        h+='<div class="cmp-st'+(t.wasteful?" waste":"")+'">'+
           '<div class="cmp-st-top"><span class="st-term" title="'+esc(t.search_term)+'">'+esc(t.search_term)+'</span>'+fl+'</div>'+
           '<div class="cmp-st-mets"><span>Spend <b>'+money(t.spend)+'</b></span><span>Clicks <b>'+num(t.clicks)+'</b></span><span>Impr <b>'+num(t.impressions)+'</b></span></div>'+
           '<div class="cmp-st-reason">'+esc(t.reason)+'</div></div>';
      });
      h+='</div>';
    }
    const rings=c.rings||[];
    h+='<div class="cmp-sec-t">Rings ('+rings.length+')</div>';
    if(!rings.length){ h+='<div class="cmp-empty">No per-ring metrics in the window.</div>'; }
    else { h+='<div class="cmp-rings">'; rings.forEach(function(r){
    ...
    const rf=body.querySelector(".cmp-refresh"); if(rf) rf.onclick=function(){ cmpDrill(c.campaign); };
    const glBtn=body.querySelector(".cmp-gl-btn");
    if(glBtn){ glBtn.onclick=function(){ cmpConfirm(c, glBtn.dataset.gl); }; }
    body.scrollTop=0;   // drill-in is navigation to a new view -> top is correct
  }

  // CONFIRM dialog for the money-gate. Shows the storm, the daily budget, and an explicit real-money
  // warning; only on the operator's confirm tap do we fire the WRITE (i_reviewed:true). Server still
  // enforces budget-sanity + campaign-must-exist, so a refusal surfaces here as states-why.
  function cmpConfirm(c, mode){
    const isGo=(mode==="go");
    const bk=document.createElement("div"); bk.className="cmp-confirm-bk";
    const warn=isGo
      ? 'This turns the campaign ON and <b>spends real money</b> \\u2014 up to the daily budget shown, every day, until you pause it. Confirm only if you have reviewed the spend.'
      : 'This pauses the campaign and stops all serving immediately. It does not delete the campaign or its Smart Bidding learning.';
    bk.innerHTML=
      '<div class="cmp-confirm">'+
        '<h3 class="'+(isGo?"go":"pause")+'">'+(isGo?"\\u26a0 Go Live \\u2014 spend real money?":"Pause campaign?")+'</h3>'+
        '<div class="cmp-cf-rows">'+
          '<div class="r"><span>Campaign</span><b>'+esc(c.campaign)+'</b></div>'+
          '<div class="r"><span>Storm</span><b>'+esc(c.date)+' \\u00b7 '+esc(c.peril||"hail")+'</b></div>'+
          '<div class="r"><span>Daily budget</span><b>'+money(c.daily_budget)+'</b></div>'+
          '<div class="r"><span>Current status</span><b>'+esc(c.status||(c.serving?"ENABLED":"PAUSED"))+'</b></div>'+
        '</div>'+
        '<div class="cmp-cf-warn">'+warn+'</div>'+
        '<div class="cmp-cf-btns"><button class="cmp-cf-cancel">Cancel</button>'+
          '<button class="cmp-cf-go'+(isGo?"":" pause")+'">'+(isGo?"Yes, go live":"Yes, pause")+'</button></div>'+
      '</div>';
    document.body.appendChild(bk);
    const close=function(){ if(typeof sgClearReshapeLayer==="function") sgClearReshapeLayer(); if(bk.parentNode) bk.parentNode.removeChild(bk); };
    bk.onclick=function(e){ if(e.target===bk) close(); };
    ...
    goBtn.onclick=async function(){
      ...
      if(res && res.ok===false){
        glFlash={campaign:c.campaign, text:"Refused \\u2014 "+(res.reason||res.states_why||res.error||"the server refused this action"), err:true}; }
      else {
        glFlash={campaign:c.campaign, text:isGo?"Campaign is now LIVE \\u2014 serving started.":"Campaign PAUSED \\u2014 serving stopped.", err:false}; }
      close(); cmpDrill(c.campaign);   // refresh so the LIVE/PAUSED state + budget reflect the change
    };
  }
  ...
  window.tempestOpenCampaigns=cmpOpen;
}
...
function flyToSearch(loc, zoom, showCard){
  if(!TMAP) return;
  ...
  const lbl = loc.label||"searched location";
  if(showCard){
    // ADDRESS PIN POPUP (2026-08-01): [address] [card icon] + Leaflet's own X. The card icon re-opens the
    // TEMP ADDRESS CARD INSTANTLY from retained state (TC_LAST) — no re-search, no re-resolve, no spinner.
    const el = document.createElement("div"); el.className = "pin-pop";
    const sp = document.createElement("span"); sp.textContent = lbl; sp.style.marginRight = "6px";
    const btn = document.createElement("button"); btn.type = "button"; btn.className = "pin-card-btn";
    btn.title = "Open the address card"; btn.innerHTML = "&#128196;";
    btn.style.cssText = "border:1px solid #c9d2dd;background:#f6f8fb;border-radius:5px;cursor:pointer;padding:1px 6px;font-size:13px";
    btn.addEventListener("click", ()=>{ if(TC_LAST) showTempCard(TC_LAST.addr, TC_LAST.lat, TC_LAST.lng, TC_LAST.opts); });
    el.appendChild(sp); el.appendChild(btn);
    SEARCH_MARKER.bindPopup(el, {autoPan:false}).openPopup();
  } else {
    SEARCH_MARKER.bindPopup(lbl, {autoPan:false}).openPopup();   // city/plain search — no card
  }
}
function addMapSearch(){
  ...
  // address-box ACTIONS: build+email the per-address storm report · add to the operator watchlist
  const repEl=document.getElementById("msReport"), watEl=document.getElementById("msWatch");
  ...
  // CARD RELAUNCH (2026-08-01): the affordance lives in the MAP PIN'S popup ([address] [card icon] + X),
  // wired in flyToSearch — NOT in the banner (banner stays global-only per the card design).
  // OPERATOR CODE (2026-08-01): one-time per-device prompt; stored in localStorage, sent as X-KCC-Operator on
  // the packet lane. Never in the repo or page source — only Netlify env (server) + this browser.
  const gear=document.getElementById("opGear");
  if(gear) gear.addEventListener("click", ()=>{
    let cur=""; try{ cur=localStorage.getItem("kcc_operator_code")||""; }catch(e){}
    const v=prompt("Operator code (bypasses INSTANT PRICE rate caps on this device). Blank to clear:", cur);
    if(v===null) return;
    try{ if(v.trim()) localStorage.setItem("kcc_operator_code", v.trim()); else localStorage.removeItem("kcc_operator_code"); }catch(e){}
    if(typeof msActMsg==="function") msActMsg(v.trim()?"Operator code saved on this device.":"Operator code cleared.");
  });
  ...
  document.querySelectorAll("#tbar .hdr-search .ms-clear").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    if(btn.dataset.clear==="msAddr"){ suggEl.innerHTML="";
      TC_LAST=null;
      if(typeof tcClose==="function") tcClose(); }               // X clears the retained card state + closes it
  ...
  async function addrGo(query){ const v=(query||addrEl.value).trim(); if(!v) return; suggEl.innerHTML="";
    ...
    if(rr && rr.ok && rr.pin_lat!=null && rr.pin_lon!=null){
      ...
      flyToSearch({lat:rr.pin_lat,lng:rr.pin_lon,label:label}, 18, true);   // rooftop pin; popup carries the card icon
...
// -- PULL (item 10, POLYGON rev 2026-08): the operator DRAWS the capture area instead of
//    aiming a fixed circle. Click PULL -> draw mode: click map points to lay a polygon
//    (live polyline + vertex dots); click near the first vertex OR press Close to seal the
//    ring into a filled polygon; Confirm COMMITS it. Committed polygons STACK - each keeps
//    a distinct colour, is listed in a side panel, and is deletable one-by-one (X in the
//    list, or click the polygon) or all at once. Pull again to add another.
//    Phase 1 = the draw/commit/manage INTERACTION only; committed rings are held in
//    window._pulls for a later phase to submit + score (see TODO(phase2)). No fixed
//    circle, no radius slider, no crosshair. Hand-rolled on vanilla Leaflet (no draw lib);
//    NEVER moves the map on its own; renderMap's section-3 core is untouched. --
function addPull(){
  const btn=document.getElementById("pullBtn"); if(!btn || !TMAP || btn.dataset.wired) return;
  btn.dataset.wired="1";
  // Committed pulls live here so a later phase can submit them. Each record:
  //   { id:string, latlngs:[[lat,lng],...] (the ring, no repeated closing point),
  //     color:"#rrggbb", layer:L.polygon }
  window._pulls = window._pulls || [];
  const PALETTE=["#f59e0b","#22d3aa","#a855f7","#ef476f","#38bdf8","#84cc16","#fb7185","#eab308"];
  let colorIx=0;
  const nextColor=()=>PALETTE[(colorIx++)%PALETTE.length];
  const SNAP_PX=13;                                            // click within 13px of vertex 0 => close ring

  // ---- draw-mode transient state (one polygon being drawn) ----
  let drawing=false, verts=[], line=null, dots=[], sealed=null, panel=null, clickH=null;

  function redrawLine(){
    if(line){ TMAP.removeLayer(line); line=null; }
    if(verts.length>=2){
      line=L.polyline(verts, {color:"#2f6fe0", weight:2, opacity:0.9, dashArray:"6,5", interactive:false}).addTo(TMAP);
    }
  }
  function clearDrawTemp(){
    if(line){ TMAP.removeLayer(line); line=null; }
    dots.forEach(d=>TMAP.removeLayer(d)); dots=[];
    if(sealed){ TMAP.removeLayer(sealed); sealed=null; }
    verts=[];
  }
  function exitDraw(){
    drawing=false; document.body.classList.remove("pull-armed");
    if(clickH){ TMAP.off("click", clickH); clickH=null; }
    clearDrawTemp();
    if(panel){ panel.remove(); panel=null; }
  }
  function renderPanel(){
    if(!panel) return;
    const txt=panel.querySelector(".pp-txt"), closeB=panel.querySelector(".pp-close"),
          goB=panel.querySelector(".pp-go"), st=panel.querySelector(".pp-status");
    if(sealed){
      txt.textContent="Polygon closed. Confirm to commit this pull, or Cancel.";
      closeB.classList.add("hidden"); goB.classList.remove("hidden");
      st.textContent="";
    }else{
      txt.textContent="Click points to draw a polygon; click the first point (or Close) to finish. ("+verts.length+" pts)";
      goB.classList.add("hidden"); closeB.classList.remove("hidden");
      closeB.disabled = verts.length<3;
    }
  }
  function closeRing(){
    if(sealed || verts.length<3) return;
    if(line){ TMAP.removeLayer(line); line=null; }
    sealed=L.polygon(verts, {color:"#2f6fe0", weight:2, opacity:0.95,
      fillColor:"#2f6fe0", fillOpacity:0.20, interactive:false}).addTo(TMAP);
    renderPanel();
  }
  // Build a committed-pull leaflet layer (reused by commit() AND loadSaved()); wires the
  // click-to-remove + tooltip. status is one of "saving"|"saved"|"error".
  function mkPullLayer(rec){
    const layer=L.polygon(rec.latlngs, {color:rec.color, weight:2, opacity:0.95,
      fillColor:rec.color, fillOpacity:0.32, interactive:true}).addTo(TMAP);
    rec.layer=layer;
    layer.on("click", ()=>removePull(rec.id));
    pullTip(rec);
    return layer;
  }
  function pullTip(rec){
    if(!rec.layer) return;
    const s=rec.status==="saved" ? "saved for "+getDate()
          : rec.status==="error" ? "NOT saved \\u2014 "+(rec.err||"error")+" (click to remove)"
          : rec.status==="saving" ? "saving\\u2026"
          : "committed pull";
    rec.layer.bindTooltip("Pull \\u2014 "+s+" \\u00b7 click to remove", {sticky:true});
  }
  // POLYGON PULL (Phase 2): submit the committed ring to the per-date write endpoint. The ring
  // is saved to getDate() (the date being VIEWED, not today). Fire-and-forget on the server
  // (the grib compute runs detached); a 202 means the write is accepted + queued.
  async function pullStore(rec){
    rec.status="saving"; pullTip(rec); renderList();
    try{
      const r=await fetch(PULL_STORE_API, { method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ date:getDate(), id:rec.id, ring:rec.latlngs }) });
      if(!r.ok){ throw new Error("HTTP "+r.status); }
      rec.status="saved";
    }catch(e){
      rec.status="error"; rec.err=(e && e.message) || "network error";
      console.error("[pull] store failed", e);
    }
    pullTip(rec); renderList();
  }
  function commit(){
    if(!sealed) return;
    const color=nextColor(), latlngs=verts.map(v=>[v[0], v[1]]);
    const id="pull_"+Date.now().toString(36)+"_"+Math.floor(Math.random()*1e4).toString(36);
    const rec={ id:id, latlngs:latlngs, color:color, layer:null, status:"saving" };
    mkPullLayer(rec);
    window._pulls.push(rec);
    pullStore(rec);                                            // SUBMIT to the per-date endpoint
    exitDraw();                                                // clears the draw temp (dots/sealed)
    renderList();
  }
  // Fetch + render the polygons already saved to the date being VIEWED. Runs on load (addPull is
  // wired once per page, and goDate() reloads the page), so returning to a date re-renders its
  // pulls; a different date shows only ITS pulls. Reuses the committed-pull styling.
  async function loadSaved(){
    let rows=[];
    try{ rows=await pquery("storm_pull_areas", { date:getDate() }); }
    catch(e){ console.error("[pull] list failed", e); return; }
    (rows||[]).forEach(row=>{
      let ring=[]; try{ ring=JSON.parse(row.ring_json||"[]"); }catch(_){ ring=[]; }
      if(!ring.length || window._pulls.some(r=>r.id===row.pull_id)) return;
      const rec={ id:row.pull_id, latlngs:ring, color:nextColor(), layer:null, status:"saved" };
      mkPullLayer(rec);
      window._pulls.push(rec);
    });
    renderList();
  ...
  function enterDraw(){
    drawing=true; document.body.classList.add("pull-armed");
    verts=[]; dots=[]; line=null; sealed=null;
    panel=document.createElement("div"); panel.id="pullPanel";
    panel.innerHTML='<span class="pp-txt"></span>'+
      '<button class="pp-close" type="button">Close</button>'+
      '<button class="pp-go hidden" type="button">Confirm pull</button>'+
      '<button class="pp-x" type="button">Cancel</button><span class="pp-status"></span>';
    document.body.appendChild(panel);
    panel.querySelector(".pp-close").onclick=closeRing;
    panel.querySelector(".pp-go").onclick=commit;
    panel.querySelector(".pp-x").onclick=exitDraw;
    clickH=(e)=>{
      if(sealed) return;                                       // ring already closed; await Confirm/Cancel
      const p=e.latlng;
      if(verts.length>=3){
        const p0=TMAP.latLngToContainerPoint(L.latLng(verts[0][0], verts[0][1]));
        const pc=TMAP.latLngToContainerPoint(p);
        if(p0.distanceTo(pc)<=SNAP_PX){ closeRing(); return; } // clicked near first vertex -> seal
      }
      verts.push([p.lat, p.lng]);
      dots.push(L.circleMarker(p, {radius:4, color:"#fff", weight:1,
        fillColor:"#2f6fe0", fillOpacity:1, interactive:false}).addTo(TMAP));
      redrawLine(); renderPanel();
    };
    TMAP.on("click", clickH);
    renderPanel();
  }

  // ---- committed-pulls list panel + deletion (backend-backed) ----
  async function pullDelete(body){
    try{
      const r=await fetch(PULL_DELETE_API, { method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify(body) });
      if(!r.ok) throw new Error("HTTP "+r.status);
    }catch(e){ console.error("[pull] delete failed", e); }   // map/local state is removed regardless
  }
  function removePull(id){
    const i=window._pulls.findIndex(r=>r.id===id); if(i<0) return;
    const rec=window._pulls[i];
    if(rec.layer){ TMAP.removeLayer(rec.layer); }
    window._pulls.splice(i,1);
    renderList();
    pullDelete({ date:getDate(), id:id });                    // delete this polygon + its cluster
  }
  function clearAll(){
    window._pulls.forEach(r=>{ if(r.layer) TMAP.removeLayer(r.layer); });
    window._pulls.length=0;
    renderList();
    pullDelete({ date:getDate(), all:true });                 // delete every polygon for this date
  }
  function renderList(){
    let el=document.getElementById("pullList");
    if(!window._pulls.length){ if(el) el.remove(); return; }
    if(!el){ el=document.createElement("div"); el.id="pullList"; document.body.appendChild(el); }
    let rows="";
    window._pulls.forEach((r,ix)=>{
      const st = r.status==="saved" ? ' \\u00b7 \\u2713 saved'
               : r.status==="error" ? ' \\u00b7 \\u26a0 not saved'
               : r.status==="saving" ? ' \\u00b7 saving\\u2026' : '';
      rows+='<div class="pl-row" data-id="'+r.id+'">'+
        '<span class="pl-sw" style="background:'+r.color+'"></span>'+
        '<span class="pl-nm">Pull '+(ix+1)+' \\u00b7 '+r.latlngs.length+' pts'+st+'</span>'+
        '<button class="pl-x" type="button" title="Remove this pull">\\u00d7</button></div>';
    });
    el.innerHTML='<div class="pl-hd"><b>Committed pulls ('+window._pulls.length+')</b>'+
      '<button class="pl-clear" type="button">Clear all</button></div>'+rows;
    el.querySelector(".pl-clear").onclick=clearAll;
    el.querySelectorAll(".pl-row").forEach(row=>{
      row.querySelector(".pl-x").onclick=()=>removePull(row.getAttribute("data-id"));
    });
  }

  btn.addEventListener("click", ()=>{
    if(drawing){ exitDraw(); return; }                         // toggle off draw mode
    enterDraw();
  });

  loadSaved();          // render the polygons already saved to the date being viewed (per-date)
}
...
// ABSOLUTE hail-size ramp (inches) — honest thresholds, NOT the relative p5/p90 auto-fit (2026-07-09
// fix: 15 sub-1" cells with max 0.84" were painted like a 2" cluster). Anchor -> color/alpha mapping:
//   < 1.125"  faint cool grey-blue, NO opacity floor (sub-gate = barely there)
//   = 1.125"  HARD STEP to green, strong opacity (the fundable gate is unmistakable)
//   1.5-1.8"  yellow -> orange   ·   >= 2.0"  red HOT   ·   >= 3.0" deep magenta-red
// A 0.84" cell can NEVER wear the color a 2" cell wears. Same anchors drive the smooth hail swath in
// BOTH fill modes (it always colors by absolute size); the §3 Cells mode + ad-circles are untouched.
const HAIL_ABS=[
  {v:0.50, c:[74,96,120],  a:0.06},
  {v:1.10, c:[96,124,158], a:0.16},
  {v:1.125,c:[40,150,96],  a:0.55},   // <- the gate step
  {v:1.50, c:[214,196,44], a:0.68},
  {v:1.80, c:[238,132,32], a:0.80},
  {v:2.00, c:[214,44,32],  a:0.90},
  {v:3.00, c:[150,12,64],  a:0.96},
];
function hailAbsColor(v){
  const A=HAIL_ABS;
  if(v<A[0].v) return null;                                   // below the faint floor -> transparent
  for(let i=A.length-1;i>=0;i--){ if(v>=A[i].v){
    if(i===A.length-1) return {c:A[i].c.slice(), a:A[i].a};
    const lo=A[i], hi=A[i+1], f=(v-lo.v)/(hi.v-lo.v);
    return {c:[Math.round(lo.c[0]+(hi.c[0]-lo.c[0])*f),Math.round(lo.c[1]+(hi.c[1]-lo.c[1])*f),Math.round(lo.c[2]+(hi.c[2]-lo.c[2])*f)],
            a:lo.a+(hi.a-lo.a)*f}; } }
  return null;
}
...
function buildHeatmap(cells, valFn, ramp, paneName, paneZ, absColorFn){
  ...
  if(absColorFn){
    // ABSOLUTE mode (hail): the blur dilutes values, so rescale the blurred grid so its PEAK maps
    // back to the true peak (max valFn over the RAW cells) -> honest absolute coloring, smooth
    // gradient preserved. Sub-gate cells stay faint; nothing sub-1.125" can wear a gated color.
    let trueMax=0; for(const c of cells){ const v=+valFn(c)||0; if(v>trueMax) trueMax=v; }
    const scale=trueMax>0 ? trueMax/gmax : 0;
    for(let y=0;y<H;y++)for(let x=0;x<W;x++){ const vAbs=G[y*W+x]*scale;
      const row=H-1-y, idx=(row*W+x)*4;                                // flip so canvas top = north
      const col=absColorFn(vAbs);
      if(!col){ img.data[idx+3]=0; continue; }                        // below the faint floor -> transparent
      img.data[idx]=col.c[0]; img.data[idx+1]=col.c[1]; img.data[idx+2]=col.c[2];
      let a=col.a; if(vAbs<0.65) a*=Math.max(0,(vAbs-0.50)/0.15);      // feather the faint fringe to nothing
      img.data[idx+3]=Math.round(255*Math.max(0,Math.min(0.96,a))); }
  } else {
...
async function boot(){
  ...
  if(TMAP){
    ...
    // The SMOOTH hail swath always colors by ABSOLUTE size (c[2] inches) via HAIL_ABS anchors — the
    // honest truth in both fill modes. A sub-gate day (all sizes < 1.125") now renders faint cool
    // presence-only (no more intensity->size fallback painting sub-gate at full color); the badge
    // states the numbers. §3 Cells mode still honors the intensity/size/priority radio (untouched).
    const hailHeat=()=>buildHeatmap(D.swath_cells,c=>+c[2]||0,HAIL_RAMP,"hHeatPane",351,hailAbsColor);
    ...
    try{ updateSwathBadge(D.swath_cells, (D.classification||{}).cls); }catch(e){} // render-truth: hail glance badge (tier word from classify.py)
  ...
  initCampaigns();      // campaign monitor: READ-ONLY live portfolio + per-campaign detail + banner (Design Lock v3 §13/§12)
  ...
  addTierLegend();          // legibility 2026-07-20: on-map tier legend (fundable/sub-gate/chase + the two boundaries)
...
def main():
    ...
    # ── LEGIBILITY PASS (2026-07-20) — two fixes whose source lines live in the ENGINE
    #    template. Applied HERE, portal-side, by the same rewrite mechanism as the pill above:
    #    engine template.html is never modified, and this dispatch is display-layer only.
    #    Each rewrite ASSERTS it matched, so an upstream template edit fails the build LOUDLY
    #    instead of silently shipping the old text (a silent no-op replace is exactly the class
    #    of defect that has bitten this project before).
    def _rewrite(src, old, new, what):
        if old not in src:
            raise SystemExit(
                "BUILD ABORTED: legibility rewrite '" + what + "' did not match the engine "
                "template.\n  looked for: " + old + "\n  The engine template changed; re-derive "
                "the rewrite rather than shipping the portal with the old text still in place.")
        return src.replace(old, new)

    # (5) The hover readout read like an error. It is not an error — it is the ordinary state
    #     of the cursor being off the swath.
    build = _rewrite(
        build,
        "readoutDiv.innerHTML=c?cellText(c):'swath: — (no cell here)';",
        "readoutDiv.innerHTML=c?cellText(c):'no mapped cells under cursor';",
        "hover readout wording")

    # (1) The market geofence and the out-of-market chase hull were near-indistinguishable:
    #     geofence #90a4ae dash 2,3 vs hull #7c869a dash 7,6 — two grey-blues, both dashed. The
    #     operator read the hull as the market boundary and concluded the map was broken. The
    #     hull is restyled hard in addChaseSwathLayer (portal-owned); the geofence becomes a
    #     COOLER, tighter, finer dotted line here so the two cannot be confused at a glance.
    #     Colour and dash ONLY — geometry, pane order and opacity semantics untouched.
    build = _rewrite(
        build,
        "style:{color:'#90a4ae',weight:1,fill:false,dashArray:'2,3',opacity:.8}",
        "style:{color:'#5b7fa6',weight:1.2,fill:false,dashArray:'1,4',opacity:.75}",
        "market geofence stroke")
